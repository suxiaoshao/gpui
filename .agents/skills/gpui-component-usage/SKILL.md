...
8. If a component reference is missing or stale, use the upstream Markdown docs fallback: `https://longbridge.github.io/gpui-kit/docs/components/{component}.md` or the full `https://longbridge.github.io/gpui-kit/llms-full.txt`.
...
- Treat `ThemeToken` as a renderable semantic background, not merely an `Hsla`.
  Apply background transforms through `.background`; using the representative
  color through deref can silently discard gradients.
...
Workspace apps use `gpui_kit::component` from GPUI Kit 0.6.0; shared crates may
retain `gpui_component`. Bundled upstream examples keep their original paths.
...
| Menu actions | `AppMenuBar`, `Command`, `Menu`, `DropdownButton` |
| Feedback and status | `Alert`, `Notification`, `Progress`, `Spinner`, `Skeleton`, `Marker`, `Shimmer`, `Badge`, `Tag`, `StatusBar` |
| Chat presentation and following | `Message`, `Bubble`, `Attachment`, `MessageScroller` |
...
- Online docs fallback: `https://longbridge.github.io/gpui-kit/docs/components/{component}.md`
- Full online docs fallback: `https://longbridge.github.io/gpui-kit/llms-full.txt`
