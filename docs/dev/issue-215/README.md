...
各 owner 按已确认版本和工作包依赖顺序实施，先生产方后消费者；root lock 由 WP-01/WP-05 协调。
删除被替换的旧调用，保留 main 后续变更；仅同步受影响的使用文档和测试示例。
若发现超出根契约的 API/行为变化，暂停该项并记录问题。
完成时核对迁移一致性、旧路径删除及既有业务/安全边界；按实际影响选择验证，同一状态不重复叠加检查。
实际证据统一写入实施记录，分别标明平台、手工 UI 和 E2E 边界。
...
具体变更、实际命令与结果、PR 记录及未验证的平台/UI/E2E 边界统一见[实施记录](implementation.md)。
