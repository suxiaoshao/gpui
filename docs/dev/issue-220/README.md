...
保留原 Issue 的模型与 thinking 选择、文本输入、流式回答、工具执行状态、失败与停止，以及标准扩展 UI 交互。普通 prompt 提交由每个会话独立持有 task：等待 RPC `prompt` 结果期间保留输入并暂时只读，发送按钮显示 loading 且阻止重复提交；成功后清空并恢复编辑，失败保留输入并显示通知以便重试，不增加失败输入恢复队列。生成期间支持 Pi 原生 steer/follow-up；扩展交互输入区仍可独立回答。按接口承接用户扩展产生的标准 RPC UI 消息，不按特定提问插件设计；不自带补协议能力的扩展。
