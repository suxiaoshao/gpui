...
启动界面由配置读取结果决定：配置不存在时提供初始设置，有效配置直达主界面；Pi 检测仅用于设置诊断。不同类型的故障进入对应恢复页面，按恢复任务划分，不为每个底层错误码建立页面。页面通过组件或函数共享结构、诊断展示和操作区；不把所有错误塞进同一个通用页面。

核心配置不可用时进入配置恢复页；配置有效即可进入主会话界面。Pi 环境与版本检测仅在设置中执行，不作为主窗口或临时会话的启动条件。会话负责查找命令、启动 RPC 以及连接失败反馈。布局缺失或不可用时使用默认窗口继续启动；不可用的 `state.toml` 直接尝试删除，退出时保存当前窗口数据，不提供布局恢复页。
...
| Pi 缺失/路径无效 | 对应会话显示连接失败；可在设置中修正路径并检查环境 | 主界面和本地历史仍可访问 |
...
| 多项启动故障 | 配置错误在配置恢复页处理；Pi 连接错误归具体会话 | Pi 版本诊断不阻塞主界面 |
...
| Q-02 | Pi 不可用时是否阻塞主界面 | 不阻塞；版本诊断留在设置，连接失败归对应会话 | D-03 |
...
1. 配置缺失时进入初始设置且保存前不自动创建文件；手动提供有效配置直接进入主界面。保存后退出重启仍可浏览本地历史；未安装 Pi 时由会话连接提示失败，并可进入设置诊断。
...
| Pi --version 超时 | 环境探测进程未按期退出 | 从提交起计算 15 秒整体 deadline，覆盖排队、查找、启动和读取；超时后释放 Child 并 Complete(Timeout)，不追加清理等待。取消通过 Cancel/Drop 停止等待；kill_on_drop 与 Tokio 负责终止及后台回收，不承诺同步阻塞立即结束或已回收；属于环境探测结果 |
