...
归属：[#217](https://github.com/suxiaoshao/gpui/issues/217)。整理日期：2026-09-16。
...
| --- | --- | --- |
| 设置字段搜索 | 已接入搜索入口和关键词；当前正式依赖 `gpui-kit/gpui-component = 0.6.0` 存在过滤后页面/分组索引错位，可能显示空白或错误页面。后续升级到**确实包含修复且兼容的版本**后，复测非首页搜索、当前页匹配/不匹配、无结果和清空搜索；不能仅因版本号提高就标记完成。2026-09-16 核对的上游 main 仍有原逻辑，最新发布 0.6.1 也不能据此当作修复版本 | [#231 设置计划](../../../app/gupi/docs/dev/issue-231/README.md)、[本地修复补丁](../../../app/gupi/docs/dev/issue-231/gpui-component-settings.patch)、[上游 #3089](https://github.com/longbridge/gpui-kit/issues/3089) |
...
| --- | --- | --- |
| 临时窗口、全局快捷翻译与托盘 | 设置基础已具备；临时窗口、选中文字/剪贴板读取、全局触发、焦点恢复及托盘生命周期尚未实施。提示词复用命令模板，不另设临时窗口专用 prompt 系统 | [#221](https://github.com/suxiaoshao/gpui/issues/221)、[#231 已确认方向](../../../app/gupi/docs/dev/issue-231/README.md#已确认方向)；开工时再建阶段计划 |
