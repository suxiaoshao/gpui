...
- [Workspace Store、Operation 与 Form 适用性调研](workspace-store-operation-form-assessment.md)：
  记录四个 app 的预定义 Operation、自定义 Transition、Store 与 Form 候选；该文档是调研快照，
  不是实施计划。
- [应用迁移决策与后续调研草稿](application-migration-decisions.md)：记录用户已经确认的行为、基于
  当前源码得到的进一步结论，以及 Conversation、Feiwen、HTTP Client 和 Novel Download
  尚未回答的问题；该文档不是实施计划。
