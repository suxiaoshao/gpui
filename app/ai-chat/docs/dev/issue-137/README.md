...
| #159 | `codex/issue-159-ai-chat2-ui`（PR #164 已合入 foundation；阶段 PR #165 已创建，尚未合入） | 在 `app/ai-chat2` 渲染项目、对话、多步 reasoning、tool、approval、多模态 timeline | 进行中：不依赖真实 agent runtime 的 foundation 已通过 PR #164 合入集成分支；当前增量已实现 Agent Conversation Page 首版，包括 New Conversation 发送后创建 conversation/user item、每会话 scratch project、sidebar 即时刷新、conversation page、已有 conversation 继续发送、真实 `AgentRuntime` observer、GPUI 原生 `ListState` / `list` timeline 和显式滚动条、user bubble、agent final markdown/details collapse、hover copy/time、Codex-style timestamp 和复制反馈；后续本地增量已把 project/conversation 的 sidebar 状态从 metadata JSON 拆到 fresh DB 列，并补齐当前 conversation 的 Codex-style stop generation。Temporary Conversation Window 首版已实现，并已把 Home-only ChatForm、picker、conversation detail/timeline 和格式化 helper 抽到 `components` / `foundation`，避免 feature 横向依赖。Prompt Settings 第一版已实现，prompt 内容模型已简化为 `prompts.content TEXT` / `PromptContent { text }`，Settings 使用管理页行列表、查看弹窗、编辑弹窗、正文多行编辑和硬删除。Shortcut Settings 第一版已实现，覆盖 fresh shortcut CRUD、runtime 注册刷新、状态诊断/重注册、selected text/clipboard 和 screenshot/OCR/image attachment 自动发送 flow。ChatForm 文件/图片附件、多模态粘贴、preview/open、attachments 表复用、Rig user image/document content 合成和 capability/runtime gating 第一版已实现，并已补 user image attachment timeline 缩略图和共享图片预览组件，记录见 `app/ai-chat/docs/dev/issue-159/chat-form-multimodal.md`。本地内置文件工具、path approval、ChatForm 审批模式 selector、tool/approval timeline row、approve/deny action、approved resume、agent streaming delta、可恢复工具错误反馈和 `ai-chat-agent` runtime/persistence 模块拆分已完成，记录见 `app/ai-chat/docs/dev/issue-159/tools-and-approval/README.md`。`config.toml` 拆分、`gpui-store` 实验性接入、依赖刷新和 repo-local GPUI skills 刷新已作为 PR #165 阶段范围提交 review。GitHub issue 仍未关闭；retry/resend、save/promote temporary flow、last item preview、Composer prompt selector、完整 project status UI、`run_command`、MCP/provider-hosted tool policy、structured output 深度 preview，以及文件附件、思考内容、生图结果等完整 rich multimodal timeline rendering 仍未完成 |
