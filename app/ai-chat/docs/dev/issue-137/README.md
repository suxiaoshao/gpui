...
| #159 | `codex/issue-159-ai-chat2-ui`（PR #164、#165、#166 已合入集成分支） | 在 `app/ai-chat2` 渲染项目、对话、多步 reasoning、tool、approval、多模态 timeline | 进行中：foundation、Agent Conversation Page、Temporary Conversation Window、Prompt/Shortcut/Skill Settings、ChatForm 多模态附件、用户图片 timeline、V1.0 本地文件工具与审批、`config.toml` 拆分、`gpui-store` 小步接入、MCP Settings/runtime/OAuth/approval resume、Composer `$skill` completion 已分阶段通过 PR #164、#165、#166 合入 `codex/issue-137-llm-abstractions`。GitHub issue 仍未关闭；合 `main` 前仍需完成当前集成分支验证基线和手动 GPUI smoke。产品剩余项主要是 retry/resend、save/promote temporary flow、last item preview、conversation title/status、Composer prompt selector、完整 project status UI、`run_command`、usage summary、structured output 深度 preview、tool progress/duration、审批 sticky preference/audit、manual provider model editor/capability override，以及文件附件、思考内容、生图结果等完整 rich multimodal timeline rendering。 |
...
- #159 详细 UI/壳/本机状态/可观测性/legacy mapping 清单见 `app/ai-chat/docs/dev/issue-159/README.md`。该文档把每项能力标为“已完成”、“占位”、“后端已具备”、“已有专项计划”、“未开始”、“暂不做”、“不照搬”或“已替代”，避免把占位入口误判为已完成业务能力。Agent Conversation Page 首版实现和剩余缺口见 `app/ai-chat/docs/dev/issue-159/agent-conversation-page.md`；Temporary Conversation Window 首版实现和剩余缺口见 `app/ai-chat/docs/dev/issue-159/temporary-window.md`；ChatForm 多模态输入实现记录见 `app/ai-chat/docs/dev/issue-159/chat-form-multimodal.md`；Skill Settings 实现记录见 `app/ai-chat/docs/dev/issue-159/skill-settings.md`；内置工具和审批计划见 `app/ai-chat/docs/dev/issue-159/tools-and-approval/README.md`。
...
- 2026-06-05 追加 Agent Conversation Page 专项计划：新增 `app/ai-chat/docs/dev/issue-159/agent-conversation-page.md`，固定 New Conversation 发送后创建 conversation、每会话匿名 scratch project、sidebar 即时刷新、conversation page、真实 `AgentRuntime` observer、canonical timeline、hover copy/time、i18n/icon/dependency 和验证计划。本次仅更新开发文档，未实现产品代码。
- 2026-06-06 已同步 live GitHub、远程分支和 Agent Conversation Page 实现状态：GitHub #137、#155-#159 仍 open；当前 `codex/issue-159-ai-chat2-ui` 暂无 PR；`origin/codex/issue-159-ai-chat2-ui` head 为 `dba4f7c`（`Implement ai-chat2 agent conversation page`），相对 `origin/codex/issue-137-llm-abstractions` 的 `git rev-list --left-right --count` 为 `1 1`，说明当前增量尚未进入集成分支。本轮实现 New Conversation 创建/打开 conversation、无项目 scratch project、已有 conversation send、真实 `AgentRuntime` observer、timeline 首版和复制/时间 UI 反馈；验证记录为 `cargo fmt`、`cargo check -p ai-chat2`、`cargo test -p ai-chat2 timestamp_label`、`cargo test -p ai-chat-agent -p ai-chat-core -p ai-chat-db` 和 `git diff --check`。
- 2026-06-08 本地开发状态追加：在 #159 分支上把 sidebar 热路径需要的 project/conversation 状态列化，`projects.pinned`、`projects.removed`、`conversations.pinned` 成为 fresh schema 和 repository API 的字段，`ProjectMetadata` / `ConversationMetadata` 不再承载这些 UI 状态；`ai-chat2` sidebar、Settings Projects 和 conversation pin/delete 流程改为读写列。由于 fresh DB 尚未合入 `main`，本次按 pre-main baseline schema 清理处理，不为旧开发期 fresh DB 追加兼容 migration。
- 2026-06-11 本地开发状态追加：在 #159 分支上补齐 Codex-style stop generation。`ChatForm` 运行中主按钮切换为 stop，点击后只停止当前 conversation 的 active run；`ConversationRuntimeStore` 为 active run 增加本地 `ActiveRunKey`、cancel token callback 和 100ms grace task，grace 后仍未自然结束时调用 `AgentRuntime::cancel_run` 把 run、active provider step 和 active tool invocation 标为 `Canceled`，移除 active run 并发 `ConversationChanged + RunFinished`；旧 task 迟到 finish 会因 key mismatch 被忽略，用户取消不写 `last_errors`。验证记录为 `cargo fmt --package ai-chat2 --package ai-chat-agent`、`cargo fmt --package ai-chat2`、`cargo test -p ai-chat-agent cancel`、`cargo test -p ai-chat2 conversation_runtime`、`cargo test -p ai-chat2 chat_form`、`cargo test -p ai-chat2`、`cargo check -p ai-chat2`、`cargo clippy -p ai-chat2 --all-targets -- -D warnings` 和 `git diff --check`。
- 2026-06-12 已同步 live GitHub 状态：#137 和 #159 仍 open；`codex/issue-159-ai-chat2-ui` 相关 PR 列表中只有已合入的 PR #164，当前远程 Prompt Settings 增量暂无新 PR；本轮功能提交为 `9f4205b`（`feat(ai-chat2): add prompt settings management`）。Prompt Settings 第一版已完成：prompt 内容模型从 JSON messages 简化为 `PromptContent { text }` 和 `prompts.content TEXT`，新增 `PromptCatalogStore` / repository CRUD，Settings Prompts 使用管理页行列表、搜索输入、显式 View/Edit/Delete、modal 查看/编辑和现有 destructive confirm；最新 UI 修正包括 Add Prompt 按钮与搜索框同高、正文编辑区固定多行高度。验证记录为 `cargo fmt`、`cargo test -p ai-chat-core prompt`、`cargo test -p ai-chat-db prompt`、`cargo test -p ai-chat-agent prompt`、`cargo test -p ai-chat2 prompts`、`cargo test -p ai-chat2 settings`、`cargo check -p ai-chat2`、`cargo clippy -p ai-chat2 -p ai-chat-core -p ai-chat-db -p ai-chat-agent --all-targets --all-features -- -D warnings` 和 `git diff --check`；未运行手动 GPUI UI 验证。
- 2026-06-13 本地开发实现追加：`app/ai-chat/docs/dev/issue-159/chat-form-multimodal.md` 已从计划更新为第一版实现记录。ChatForm 已实现文件/图片附件 strip、GPUI clipboard files/image paste、`+` 附件菜单、GPUI `ExternalPaths` 文件拖拽到 ChatForm、Codex app reference 样式常量、attachments 表复用、capability/runtime gating、图片 app 内 preview、macOS Finder 级 Quick Look file preview fallback、i18n/icons；`ai-chat-agent` history 已把 image/PDF/text-like file attachments 合成为 Rig user multimodal content。provider file upload、audio/generated output 和完整 rich multimodal timeline 仍留后续。
- 2026-06-14 本地开发状态追加：ChatForm 多模态代码已完成局部结构整理，`chat_form.rs` 保留整体状态、提交和 root layout，附件操作流拆到 `components/chat_form/attachment_flow.rs`，附件 strip/menu/card/warning 拆到 `components/chat_form/attachment_views.rs`，图片全窗口预览当时保留在 `components/chat_form/image_preview.rs`，后续已提升为共享 `components/image_preview.rs`。补充验证为 `cargo fmt --check`、`cargo check -p ai-chat2`、`cargo test -p ai-chat2 chat_form`、`cargo test -p ai-chat-agent history`、`cargo test -p ai-chat-db attachment`、`cargo test -p ai-chat-db typed_json_roundtrips_for_repository_records`、`cargo clippy -p window-ext -p ai-chat2 -p ai-chat-db -p ai-chat-agent --all-targets --all-features -- -D warnings` 和 `git diff --check`。
- 2026-06-15 本地开发状态追加：图片预览组件已提升为共享 `components/image_preview.rs`，ChatForm 附件和 conversation timeline 用户图片共同复用；预览控件改用 `popover` / `popover_foreground` / `muted_foreground` / `border` theme token。Conversation timeline 已补 user image attachment 缩略图和点击预览；文件附件、tool/MCP 结果、思考内容、生图结果等完整 rich timeline 仍留后续。补充验证为 `cargo fmt`、`cargo check -p ai-chat2`、`cargo test -p ai-chat2 chat_form`、`cargo test -p ai-chat2 user_image_attachments` 和 `git diff --check`。
- 2026-06-16 本地开发状态追加：`app/ai-chat/docs/dev/issue-159/tools-and-approval/plan.md` 已从计划更新为实现记录。V1.0 本地文件工具、ripgrep-backed `grep`、path approval、ChatForm 审批模式 selector、tool/approval timeline row、approve/deny action、approved resume、真正 streaming delta、可恢复工具错误反馈已完成；`ai-chat-agent` 已把 `runtime.rs` 拆为 `runtime/streaming.rs`、`runtime/finalization.rs`、`runtime/approval_resume.rs`、`runtime/tests.rs`，把 `persistence.rs` 拆为 `persistence/model.rs`、`persistence/provider_step.rs`、`persistence/conversation_items.rs`、`persistence/tool_hook.rs`。仍未完成 `run_command`、MCP/provider-hosted source-specific policy、structured output 深度 preview、tool progress/duration、tool name collision display、跨重启审批 sticky preference 和完整 rich multimodal timeline。验证记录为 `cargo fmt`、`cargo test -p ai-chat-core tool_policy`、`cargo test -p ai-chat-agent builtin_tools`、`cargo test -p ai-chat2 approval_mode`、`cargo test -p ai-chat-agent streaming`、`cargo test -p ai-chat-agent recoverable`、`cargo test -p ai-chat-agent approval`、`cargo test -p ai-chat-agent tool_error_output_is_persisted_without_reconstructing_from_model_text`、`cargo test -p ai-chat-agent`、`cargo check -p ai-chat-agent -p ai-chat2`、`cargo clippy -p ai-chat-agent -p ai-chat2 --all-targets -- -D warnings` 和 `git diff --check`。
- 2026-06-17 本地开发状态追加：完成 `config.toml` 第一批拆分。`AppSettingsPayload` 已从 fresh DB `app_settings` 表迁到 `app/ai-chat2` 的 `[app_settings]` config section；`config.toml` 同时新增 `[mcp_servers.<id>]` 声明式 MCP server 配置入口，可解析 stdio / streamable HTTP 并转换为 `McpConfigLayer`，但本批不连接 server、不注册 MCP tools、不新增 MCP UI。fresh DB baseline 已移除 `app_settings` 表；`providers`、`provider_models`、`prompts`、`shortcuts` 仍保持现有 DB-backed 边界，作为后续候选拆分项。
- 2026-06-17 本地开发状态追加：`config.toml` 第二批小步补齐 ChatForm composer defaults。新增 `[chat_form]`，保存当前 provider/model 选择、reasoning selection 和 tool approval mode；ChatForm 启动时优先消费 config，失效时回退到当前 enabled model，并在模型/思考强度/审批模式变化时写回 config。provider definitions、provider model fetch cache、manual model editor 和 capability override 仍保持后续批次边界。
- 2026-06-18 本地开发状态修正：`crates/gpui-store` 仍是实验性通用 crate，但已开始在 `app/ai-chat2` 做小步接入：`config.toml` 由 `AiChat2ConfigStore = SharedStore<AiChat2Config, AiChat2ConfigSource>` 成为唯一 config global，`AiChat2AppSettings` 退化为派生只读视图；`PromptCatalogStore` 已改为 `SharedStore<PromptCatalogState, PromptCatalogSource>` 的 DB projection，Prompt Settings 直接订阅 prompt rows snapshot。暂不做 wholesale migration；`state.toml`、providers、projects、workspace sidebar 和 conversation runtime/timeline 仍保持现有边界，后续逐个评估是否迁移。
- 2026-06-14 ChatForm 附件 polish 和 macOS 文件对话框本地化追加：`+` 附件入口已改用 `gpui-component` `DropdownMenu` / `PopupMenuItem`，不再由 `ChatForm` 持有菜单 open state；图片缩略图改为无 padding/margin/1px inset 的专用 0 inset rounded 裁剪层 + 0 inset border overlay；添加文件对话框保留 `PathPromptOptions.prompt = None`，并通过 `xtask` macOS bundle localization 输出 `en.lproj` / `zh_CN.lproj`、`CFBundleAllowMixedLocalizations = true` 修正 `NSOpenPanel` 系统 UI 语言。补充验证为 `cargo fmt`、`cargo check -p ai-chat2`、`cargo test -p ai-chat2 chat_form`、`cargo test -p xtask bundle`、`git diff --check`、`cargo run -p xtask -- bundle ai-chat2` 和生成 bundle plist/resources 检查。
- 2026-06-14 provider capability profile 追加：ChatForm 多模态 gating 的上游模型能力画像已补齐。OpenAI GPT-5.5/GPT-5.4/GPT-5/GPT-4o/GPT-4.1/o3/o4 现在标记 image/file input；Claude 当前模型和 Gemini 2.5/3 标记 image/file input；OpenRouter 读取 `/api/v1/models` 的 `architecture.input_modalities` 得到 image/file；DeepSeek v4/chat/reasoner 补齐 tool/structured output；Mistral 补齐 tool/structured output，但由于当前 Rig Mistral adapter 明确不支持 `UserContent::Image`，Mistral vision models 暂不对 ChatForm 放开 image input。补充验证为 `cargo fmt`、`cargo test -p ai-chat-agent model_capabilities` 和 `cargo test -p ai-chat-agent provider_models`。
- 2026-06-20 已创建阶段 PR #165 `ai-chat2: stage agent UI, tools, and gpui-store integration`，从 `codex/issue-159-ai-chat2-ui` 指向 `codex/issue-137-llm-abstractions`。PR 创建时功能 head 为 `d0c2217`，覆盖 PR #164 后的 24 个功能提交：Agent Conversation Page、Temporary Conversation Window、Prompt/Shortcut Settings、ChatForm 多模态附件、用户图片 timeline、内置文件工具和审批、`config.toml` 拆分、`gpui-store` 小步接入、依赖刷新和 repo-local GPUI skills 刷新；PR 创建后追加本文档状态同步提交。提交 PR 前补充验证为 `cargo fmt --check`、`git diff --check origin/codex/issue-137-llm-abstractions...HEAD`、`cargo check -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db -p gpui-store`、`cargo test -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db -p gpui-store`、`cargo clippy -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db -p gpui-store --all-targets --all-features -- -D warnings`；未运行 full workspace build/test/clippy 或手动 GPUI UI 验证。
- 2026-06-23 Skill Settings 第一版已实现：Settings 新增 `Skills` 分栏，展示全局 skill catalog，提供刷新、搜索和列表内展开查看 `SKILL.md` 原始内容；不做 enable/disable、安装/卸载、编辑或数据库 source table。验证为 `cargo fmt`、`cargo test -p ai-chat2 skill`、`cargo test -p ai-chat2 settings`、`cargo check -p ai-chat2`、`cargo clippy -p ai-chat2 --all-targets -- -D warnings` 和 `git diff --check`。
- 2026-06-25 MCP Settings/runtime V1 已在 2026-06-24 状态页和非 OAuth runtime 基础上补齐管理 UI：`app/ai-chat2` 新增 config-backed MCP Settings 页，读取 `config.toml` `[mcp_servers]`，支持搜索、刷新/测试、连接状态、auth 状态、server info、tools/list 展示，以及 Add/Edit/Delete/Enable/Disable 管理能力并写回 `config.toml`；`state::mcp` 安装 app-global `McpRuntimeStore`，通过 `ai-chat-agent::McpSessionManager` 按需连接 stdio / 非 OAuth streamable HTTP server，并在 agent run `begin_run` 前注册 MCP tools。MCP definitions 只保存在 `config.toml`，live sessions/tool cache 只保存在 runtime，agent run DB 不保存 MCP config/hash。OAuth browser flow/token storage/refresh/scope upgrade/logout、OAuth 配置表单、ClientCredentials UI、Codex-style prewarm 和 MCP source-neutral approval resume 已明确记录为后续项，详见 `app/ai-chat/docs/dev/issue-159/mcp/README.md`。
- 2026-06-25 MCP Add/Edit UI 计划已按 Codex 自定义 MCP 表单重新收敛：默认表单只暴露名称、stdio 启动命令/参数/env/env_vars/cwd 和 streamable HTTP URL/bearer token env/header/env header；参数、环境变量和 headers 全部改为结构化数组/键值 rows，保存时做必填与格式校验并 merge 回 `config.toml`，保留 TOML-only 的 OAuth、required、timeouts、tool filters 和 approval overrides。详细模块、类型、数据流、i18n、icon、依赖和验证计划见 `app/ai-chat/docs/dev/issue-159/mcp/README.md`。
- 2026-06-22 PR #165 `ai-chat2: stage agent UI, tools, and gpui-store integration` 已合入 `codex/issue-137-llm-abstractions`，merge commit 为 `e880005`。
- 2026-06-27 PR #166 `ai-chat2: stage MCP and skill integration` 已合入 `codex/issue-137-llm-abstractions`，merge commit 为 `0e11058`。该阶段覆盖 Skill Settings、Composer `$skill` completion、MCP Settings/runtime、streamable HTTP/OAuth、MCP tool registration、MCP approval resume、same-run approval gating 和相关稳定性修复。PR 验证记录为 `git diff --check origin/codex/issue-137-llm-abstractions...HEAD`、`cargo fmt --check`、`cargo check -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db`、`cargo test -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db`、`cargo clippy -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db --all-targets --all-features -- -D warnings`；未运行 full workspace build/test/clippy 或手动 GPUI UI 验证。
- 2026-06-27 live GitHub 状态：#137、#155、#156、#157、#158、#159 仍 open；当前没有 open PR 指向 `codex/issue-137-llm-abstractions`。
- 2026-06-27 集成分支 focused baseline 已完成：`git diff --check`、`cargo fmt --check`、`cargo check -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db -p gpui-store`、`cargo test -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db -p gpui-store`、`cargo clippy -p ai-chat2 -p ai-chat-agent -p ai-chat-core -p ai-chat-db -p gpui-store --all-targets --all-features -- -D warnings` 均通过。过程中修正了 PR #166 后新 Rust/Clippy 版本暴露的 lint：MCP test helper 的 useless `vec!`、test-only OAuth legacy key helper 的非测试构建 dead code、以及 MCP credentials key fallback 的 `map_or_else` 写法。仍未运行 full workspace build/test/clippy 或手动 GPUI UI 验证；Cargo 仍提示依赖 `block v0.1.6` 的既有 future-incompat warning。
...
- OpenAI model classification 现在输出 Responses API、reasoning effort、image/file input、structured output、stateful response continuation 等 typed capabilities；provider-hosted Web/File search 和 Computer Use 等工具仍需 runtime 接线后再打开对应 capability。
- Anthropic、Gemini、DeepSeek、Mistral 和 OpenRouter 的 provider model classification 已区分官方/平台声明与当前 runtime adapter 能力：Claude/Gemini/OpenRouter 可对 ChatForm 放开图片/文件输入；DeepSeek 只补 tool/structured output；Mistral 暂不标记 image input，直到 provider adapter 能发送图片。
...
#155 的事实来源是 `app/ai-chat/docs/dev/issue-155/fresh-database-schema.md`。
...
现有 #155-#159 已经创建，并且已在 2026-05-23 按下面的边界同步到 GitHub title/body。后续如果继续拆分或改名，应先更新本文档和 `app/ai-chat/docs/dev/issue-155/fresh-database-schema.md`，再同步线上 issue。
...
- `app/ai-chat2` 需要实现 project-first navigation、canonical `conversation_items` timeline、prompt/provider/model composer、agent run/retry/resend、tool/approval/status/usage/multimodal rendering、settings 和 shortcut UI。
- 已合入的 foundation 完成基础设施壳、app chrome、file-backed logging、About、Home/Sidebar skeleton、ChatForm 视觉预览、`ComposerEditor` 第一版输入能力、Settings shell + General/Appearance/Projects、New Conversation 默认页、no-project 项目选择器、Codex-style composer/project tray polish、main/settings window placement 和基础 parity 修复；Provider settings 第一阶段已实现，已具备 fresh DB provider/model 基础 UI、Provider i18n、默认 disabled、独立滚动布局、保存前本地校验、未保存状态标签、GPUI credentials secret write/read、真实 model fetch、`gpui_tokio` runtime bridge、`ListState` provider/model lists、provider list panel/row separator 和右侧 detail 整体滚动；Composer DB-backed provider/model picker 已实现，且 provider model cache 已开始保存 capability source 和 provider-specific reasoning control；Provider brand assets 已完成第一版，包含 `ProviderLogoName` / `ProviderVisual`、Simple Icons-first vendored SVG、generic Lucide fallback 和 `crates/app-assets-macros` 过程宏拆分；sidebar 的 project/conversation pin/remove 状态已从 metadata JSON 拆为 fresh DB columns；Agent Conversation Page 已补当前 run stop generation；Temporary Conversation Window 首版已实现，并已把 Home-only ChatForm、picker、conversation detail/timeline 和格式化 helper 抽到共享层；Prompt Settings 第一版已实现，采用 `prompts.content TEXT` 简化模型、管理页行列表、modal 查看/编辑和硬删除；Shortcut Settings 第一版已实现，覆盖 fresh shortcut CRUD、runtime 注册刷新、状态诊断/重注册、selected text/clipboard 和 screenshot/OCR/image attachment 自动发送 flow；Skill Settings 和 Composer `$skill` completion 已实现；ChatForm 多模态输入第一版已实现，覆盖附件 strip、粘贴/拖拽/文件选择、preview/open、attachments 表复用、agent history 多模态 content 和 gating，且附件菜单、图片缩略图圆角边框、macOS 文件对话框 bundle localization 已完成 polish；conversation timeline 已补 user image attachment 缩略图显示和共享图片预览；V1.0 内置文件工具、approval action、tool/approval timeline row、streaming delta、可恢复工具错误反馈、MCP Settings/runtime/OAuth、MCP tool registration、MCP approval resume 和 same-run approval gate 已实现；manual provider model editor、retry/resend、save/promote temporary flow、`run_command`、structured output 深度 preview、usage summary、tool progress/duration、完整 rich multimodal timeline rendering 和更完整真实 OAuth MCP server 手动验证仍未完成。
- 详细清单和旧 `app/ai-chat` 对比见 `app/ai-chat/docs/dev/issue-159/README.md`；agent conversation page 的 implementation-ready 计划见 `app/ai-chat/docs/dev/issue-159/agent-conversation-page.md`；temporary window 的实现记录见 `app/ai-chat/docs/dev/issue-159/temporary-window.md`，其模块计划禁止 Temporary 横向依赖 Home feature；Prompt Settings 计划见 `app/ai-chat/docs/dev/issue-159/prompt-settings.md`；Skill Settings 计划见 `app/ai-chat/docs/dev/issue-159/skill-settings.md`；Shortcut Settings 计划见 `app/ai-chat/docs/dev/issue-159/shortcut-settings.md`；ChatForm 多模态输入实现记录见 `app/ai-chat/docs/dev/issue-159/chat-form-multimodal.md`；内置工具和审批计划见 `app/ai-chat/docs/dev/issue-159/tools-and-approval/README.md`。
...
#158 已通过 PR #163 完成 `ai-chat-agent` 的 Rig/rmcp agent runtime、file-backed skills、MCP/tool/approval runtime 和 fresh persistence 写入，并合入集成分支。当前实现子 issue 是 #159。`codex/issue-159-ai-chat2-ui` 已推进 `ai-chat2` 基础设施壳、app chrome、file-backed logging、About、Sidebar/home skeleton、Home root/sidebar 结构修正、ChatForm 视觉预览、`ComposerEditor` 第一版输入内核、cursor/scroll 修正、Unicode/grapheme-aware 编辑、Settings shell + General/Appearance/Projects、New Conversation 默认页、no-project 项目选择器、Codex-style composer/project tray polish、基础 parity 修复、Provider settings 第一阶段实现（保存校验、secret credentials、真实 model fetch、`gpui_tokio` bridge、`ListState` provider/model lists、provider list panel/row separator、右侧 detail 整体滚动）、DB-backed Composer provider/model picker、provider model capability source / reasoning control 第一版、provider brand assets / app-assets 过程宏 refactor，以及 project-first sidebar 第一版；这些不依赖真实 agent runtime 的 foundation 已通过 PR #164 合入集成分支。当前增量已实现 Agent Conversation Page 首版：真实 conversation create/send、scratch project、agent run 启动、runtime observer 和 canonical timeline v1 已接线，并已把 timeline 对齐为 GPUI 原生 `ListState` / `list` + `vertical_scrollbar`；同时已把 sidebar 热路径的 project/conversation pin/remove 状态从 metadata JSON 拆到 fresh DB 列，并补齐当前 conversation 的 stop generation。Temporary Conversation Window 首版已实现，并已把 Home-only ChatForm、picker、conversation detail/timeline 和格式化 helper 抽到 `components` / `foundation`；Prompt Settings 第一版已实现，prompt 内容模型已从 JSON messages 简化到 `PromptContent { text }` 和 `prompts.content TEXT`，并接入管理页行列表、modal 查看/编辑、正文多行编辑和 Settings CRUD。Shortcut Settings 第一版已实现，覆盖 fresh shortcut CRUD、runtime 注册刷新、状态诊断/重注册、selected text/clipboard 和 screenshot/OCR/image attachment 自动发送 flow。ChatForm 文件/图片附件、多模态粘贴、preview/open、attachments 表复用、Rig user image/document content 合成和 capability/runtime gating 第一版已实现，并已补齐 conversation timeline 用户图片附件缩略图和共享图片预览组件，记录见 `app/ai-chat/docs/dev/issue-159/chat-form-multimodal.md`。V1.0 内置文件工具、path approval、ChatForm 审批模式、tool/approval timeline row、approve/deny action、approved resume、streaming delta 和可恢复工具错误反馈也已完成，记录见 `app/ai-chat/docs/dev/issue-159/tools-and-approval/README.md`。后续仍应继续 #159 的 Composer prompt selector、retry/resend、save/promote temporary flow、last item preview、完整 project status UI、`run_command`、MCP/provider-hosted tool policy、structured output 深度 preview，以及文件附件、思考内容、生图结果等完整 rich multimodal timeline rendering，而不是回到旧 `messages` table 或在 GPUI 层重写 agent loop。#155 已把 `ai-chat2` 并行重构、crate 拆分和后续 issue 重排固定到设计文档中。除非明确重新定义 scope，否则 #154 不应作为旧 `messages` table 上的窄补丁继续推进。
