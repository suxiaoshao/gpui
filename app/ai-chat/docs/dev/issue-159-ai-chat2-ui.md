...
| canonical timeline | `conversation_items` | 已实现首版：Conversation page 按 snapshot 渲染 GPUI 原生 `ListState` / `list` timeline、显式滚动条、user bubble、user image attachment 缩略图、agent final markdown/details collapse 和 observer invalidation。tool/MCP/approval/reasoning/generated output 等 rich timeline 后续继续补。 |
