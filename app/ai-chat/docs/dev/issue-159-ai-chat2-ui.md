...
`RunFinished`。2026-06-13 本地增量已实现 ChatForm 文件/图片附件第一版：附件 strip、`+` 菜单、GPUI clipboard files/image paste、Finder/系统文件 drag/drop、图片 app 内 preview、macOS Quick Look 文件 preview fallback、attachments 表复用、Rig 多模态 user content 合成和 capability/runtime gating。2026-06-14 已完成 ChatForm 多模态局部结构整理：附件操作流、附件视图、附件常量和图片预览收敛到 `chat_form/*` 子模块，`chat_form.rs` 保留整体状态、提交和 root layout 编排。完整多模态 timeline、manual provider model editor、retry/resend、approval action、rich tool UI 仍未完成。
...
- 本轮实现：ChatForm 文件/图片附件第一版（clipboard/drop/file picker、附件 strip、图片全窗口 preview、macOS Quick Look file preview、attachments 表复用、Rig image/document content、capability/runtime gating）和 ChatForm 多模态模块拆分
...
| attachments | `attachments` + typed payloads | 第一版已实现：ChatForm 文件/图片输入、GPUI clipboard 分类、drag/drop、preview/open、attachments 表复用、`ConversationTimelineRecords.attachments`、Rig user image/document content 合成和 unsupported-state messaging。audio input、generated output、provider file upload 和完整 timeline multimodal rendering 仍未开始。详见 `app/ai-chat/docs/dev/issue-159-ai-chat2-chat-form-multimodal.md`。 |
...
| prompts | `prompts` | Prompt Settings 第一版已实现：`prompts.content_json JSON` 改为 `content TEXT`，`PromptContent` 收敛为简单文本，Settings 新增 Prompts 管理页并提供搜索、管理行、modal 查看、新增、编辑、正文多行编辑、硬删除；Shortcut prompt binding 已由 Shortcuts Settings 第一版接入，Composer prompt selector 后续继续补。详见 `app/ai-chat/docs/dev/issue-159-ai-chat2-prompt-settings.md`。 |
...
| app settings | `app_settings` | General/Appearance 已消费 language、theme、temporary hotkey 和 HTTP proxy；New Conversation 默认页已消费并更新/清空 default project；Provider settings 第一阶段已落地并有专项文档继续跟踪；Prompt Settings 第一版已接入；Shortcut Settings 第一版已接入 fresh shortcuts 表。 |
...
- shortcut 触发的 selected text/clipboard/screenshot 输入已在 Shortcuts Settings 第一版接线；仍未做 save/promote to normal conversation、retry/resend、approval action、
