...
| 能力 | `ai-chat2` 状态 | 说明 |
| --- | --- | --- |
| app-local config | 已完成 | `config.toml` 保存启动层本机配置、`[app_settings]` 全局偏好、`[chat_form]` composer 默认模型/思考强度/审批模式，以及 `[mcp_servers.<id>]` 声明式 MCP source definitions；写入口仍由 `state::config` helper 手写更新 `AiChat2Config` global 并 `save()`，no-op update 不改写文件；业务 transcript/runtime 仍在 fresh DB。 |
...
| prompts | `prompts` | Prompt Settings 第一版已实现：`prompts.content_json JSON` 改为 `content TEXT`，`PromptContent` 收敛为简单文本，Settings 新增 Prompts 管理页并提供搜索、管理行、modal 查看、新增、编辑、正文多行编辑、硬删除；`PromptCatalogStore` 仍是 event facade，create/update/delete 直接走 repository command 并发 `PromptCatalogEvent`；Shortcut prompt binding 已由 Shortcuts Settings 第一版接入，Composer prompt selector 后续继续补。详见 `app/ai-chat/docs/dev/issue-159-ai-chat2-prompt-settings.md`。 |
