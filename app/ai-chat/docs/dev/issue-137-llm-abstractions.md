...
- 2026-06-18 本地开发状态修正：新增的 `crates/gpui-store` 仅保留为实验性通用 crate 和设计文档，不再接入 `app/ai-chat2`。先前尝试的 config、`state.toml`、prompts、providers、projects、workspace sidebar 和 conversation timeline store 接入已撤回；`app/ai-chat2` 当前仍使用手写 `state::config` helper、`AiChat2LayoutState`、repository-backed catalog event facade 和页面本地 state。后续如需接入 store，必须在 `gpui-store` 设计稳定后另起应用专项计划。
