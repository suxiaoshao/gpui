...
| #159 | `codex/issue-159-ai-chat2-ui`（PR #164 已合入 foundation；当前远程 head `dba4f7c` 是 Agent Conversation Page 增量，暂无 PR） | 在 `app/ai-chat2` 渲染项目、对话、多步 reasoning、tool、approval、多模态 timeline | 进行中：不依赖真实 agent runtime 的 foundation 已通过 PR #164 合入集成分支；当前增量已实现 Agent Conversation Page 首版，包括 New Conversation 发送后创建 conversation/user item、每会话 scratch project、sidebar 即时刷新、conversation page、已有 conversation 继续发送、真实 `AgentRuntime` observer、`gpui-component::List` timeline、user bubble、agent final markdown/details collapse、hover copy/time、Codex-style timestamp 和复制反馈；后续本地增量已把 project/conversation 的 sidebar 状态从 metadata JSON 拆到 fresh DB 列。GitHub issue 仍未关闭；cancel/retry/resend、approval action、attachments/multimodal input、rich tool UI、Temporary Conversation runtime、last item preview 和完整 project status UI 仍未完成 |
