...
- 2026-06-03 本地开发状态追加：Provider settings 后续已完成 DB-backed Composer provider/model picker、provider model capability source 建模、Ollama/Gemini/OpenRouter API-discovered capability enrichment、OpenAI/Anthropic/DeepSeek/Mistral docs-derived reasoning profiles、`ReasoningControl`/`ReasoningSelection` payload、Composer token budget selector，以及 provider-specific reasoning additional params 生成。该条是本地开发文档更新，不代表重新同步过 live GitHub issue/PR 状态。
- 2026-06-03 Provider brand icon 决策：Lucide v1 已移除品牌图标，provider logo 后续不能作为 app-local Lucide `IconName` 增补；应使用 app-owned runtime brand SVG asset，来源优先为各品牌官方 asset 或 Simple Icons，并保留 generic Lucide fallback。当前 ChatForm 和 Settings provider row 仍使用 `Cpu`/`Cloud`/`Server` 等通用图标，provider brand logo 尚未实现。
