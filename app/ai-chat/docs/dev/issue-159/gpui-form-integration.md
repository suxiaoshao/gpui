...
以 `gpui-form` 的 `binding-architecture.md` 为准：number 是 `State -> Draft -> Result<Value, FieldError>`
...
pub(super) struct OllamaProviderFormInput {
    ...
    #[form(binding = "StringInputBinding", label = "provider-field-bearer-token", placeholder = "provider-placeholder-bearer-token", mask)]
...
pub(super) struct CustomOpenAiProviderFormInput {
    ...
    #[form(binding = "StringInputBinding", label = "provider-field-api-key", placeholder = "provider-placeholder-api-key", required, mask)]
    pub api_key: String,
...
- `Input` / `InputState`：provider text/secret/base URL，MCP text fields 和 row fields；后续通过
...
- `NumberInput`：ChatForm token budget 继续由 ChatForm 自己使用，P3 前不下沉到 Settings form；若后续进入
  generated form，使用 adapter `NumberInputBinding<N>`，其 `Draft = String`。
...
1. 已完成：Provider/MCP 保存、校验和测试 helper 改为以 generated form `draft()` 为业务数据源，组件 state
   不再作为提交结构的读取来源。
