...
`select`、`combobox` 和 bool 字段都已通过 binding 创建和同步组件 state。Provider/MCP/Prompt/Shortcut
的提交数据源都已改为 generated form `draft()`，组件 state 只作为 UI 渲染和输入事件来源。MCP Add/Edit dialog
...
- 写 SQLite、`config.toml`、GPUI credentials/keychain 或刷新 runtime store。
...
| --- | --- | --- | --- |
| Provider Settings | 已完成主要表单层迁移：每个 provider kind 是确定的 generated store，组件 state、required、field errors 和 typed form events 都由 `gpui-form` 承载。 | `ProviderDraft` 兼容层、secret ref/dirty 语义、DB/keychain 保存、model refresh、enabled model toggle。 | 无动态 field schema 残留；`write_to_draft` 只作为保存前业务 payload 同步，不算表单层残留。 |
| MCP Add/Edit Dialog | 已完成字段值、required、array row identity、add/remove、placeholder 和保存前 field errors 迁移。 | `McpServerTomlConfig` merge、OAuth draft promotion、config 写入、runtime status/tools 展示。 | 无；validator issues 已映射到 generated form field errors，summary 从 form errors 派生。 |
...
- `Select` / `SelectState` 用于 provider API mode，以及内置 binding 能表达的 shortcut prompt/model choices。
...
- submit/validate/业务 payload 构造都从 generated form `draft()` 或 `ProviderSettingsForm` /
...
pub(super) struct CustomOpenAiProviderFormInput {
    ...
    #[form(component = "input", label = "provider-field-name", placeholder = "provider-placeholder-provider-name", required)]
    pub name: String,

    #[form(component = "input", label = "provider-field-api-key", placeholder = "provider-placeholder-api-key", required, mask)]
    pub api_key: String,

    #[form(component = "input", label = "provider-field-base-url", placeholder = "provider-placeholder-custom-base-url", required)]
    pub base_url: String,
...
- API-key provider 和 Custom OpenAI-compatible 的 `api_key` 是 required secret；saved secret ref 可以满足
  required，清空 required secret 后保存必须失败。
...
pub(super) struct McpServerFormInput {
    ...
    #[form(component = "input", label = "mcp-field-name", placeholder = "mcp-placeholder-server-id", required)]
...
DB provider row + model rows + ProviderCatalog + GPUI credentials existence
  ...
  -> submit reads form.draft() and app normalizes field values for payload
  -> app-specific validation maps issues to generated form field errors
...
config.toml [mcp_servers.<id>]
  ...
  -> submit reads form.draft() and app normalizes field values for config payload
  -> MCP validator maps issues to generated top-level/row field errors
  -> McpServerTomlConfig
...
- `gpui-form` 不持有保存任务；成功 submit 后的 DB/config/keychain/runtime 更新全部由 app 调用现有
  state/repository/config helper。
- Provider 成功 submit 后才写 fresh DB provider row 和 GPUI credentials；失败时只更新表单错误。
- MCP 成功 submit 后才写 `config.toml`；失败时只更新表单错误。
- Prompt 成功 submit 后才写 prompts 表；duplicate name 在 app validator 前置，DB UNIQUE 继续兜底。
- Shortcut 成功 submit 后才写 shortcuts 表；provider/model snapshot 仍由 state 层生成，DB nullable FK 不改变
  UI required 语义。
...
本批迁移不新增依赖。Provider、MCP、Prompt 和 Shortcut 的 required / URL / duplicate / hotkey 校验都复用
现有 app validator 与已有依赖；`gpui-form` 继续作为 workspace 依赖提供 form store、binding 和 error helpers。
...
12. 已完成：补齐 `gpui-form` required 字段支持。
    ...
    - `ComponentStateOptions` 传递 required 给 binding；内置 binding 不渲染 marker。
...
14. 已完成：迁移 Shortcut Edit Dialog。
...
15. 已完成：收敛 MCP Add/Edit dialog 的错误状态。
...
16. 可选 P2：在 Shortcut 的 hotkey binding 稳定后，再评估 General Settings 的 temporary hotkey inline editor。
17. 可选 P2：HTTP proxy 输入可以迁移为 single-field auto-save form，但必须保留 config 写入失败时不覆盖
    `last_value` 的语义；迁移前必须读取实际 runtime 使用方和 URL scheme 白名单。
18. 暂缓 P3：ChatForm controls 需要单独设计，不和 Settings 表单迁移混做。
