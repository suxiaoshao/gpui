...
`app/ai-chat/docs/dev/issue-159-ai-chat2-composer-model-picker.md`；它只读取 enabled provider/model
cache，不读取 keychain，不启动 agent run。尚未完成 manual model editor、provider brand logo、
prompt/provider/model 运行时接线、完整 Rig completion client factory 和真实 agent run。
