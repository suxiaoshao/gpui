...
`app/ai-chat2` 当前只在 `ComposerEditor` 内直接调用 `SkillCatalog::scan(...)`：

- composer 无项目时扫描 global catalog。
- New Conversation / Conversation Detail 有 project root 时刷新 project-aware catalog。
- 目前没有 Settings skill catalog UI，也没有 app-level skill store。
...
- Composer 后续应从 `state::skills` 读取 global catalog，project-aware catalog 仍需要额外 project root overlay；本阶段不强制迁移 Composer。
