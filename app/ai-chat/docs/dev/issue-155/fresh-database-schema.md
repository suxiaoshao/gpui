...
- app 全局偏好、ChatForm 默认选择、启动层路径和 MCP server 定义属于 `config.toml`，不是 fresh database source tables。当前已把 `AppSettingsPayload` 移到 `[app_settings]`，新增 `[chat_form]` 保存 composer 默认模型/思考/审批偏好，并预留 `[mcp_servers.<id>]`。
- 本机 UI layout/window state 属于 `state.toml`，不是 fresh database source table；当前 `app/ai-chat2` 由 `AiChat2LayoutState` 启动加载、debounce 保存和 quit 前同步 flush，保存 Sidebar 宽度和 main/settings window placement。
...
| `projects` | 真实文件夹项目，包含 scratch 项目 | `id`, `path UNIQUE`, `display_name`, `kind`, `pinned`, `removed`, 时间戳 | `ProjectMetadata` |
| `conversations` | 对话元数据，每个对话必须属于一个项目 | `id`, `project_id`, `title`, `status`, `pinned`, `last_item_seq`, 时间戳 | `ConversationMetadata`, `ConversationSettingsSnapshot` |
...
| `prompts` | 保存的系统/开发者提示词 | `id`, `name UNIQUE`, `content`, `enabled`, `sort_order`, 时间戳 | `PromptContent` |
...
- MCP server 配置保存在 app/user/project config 中，例如 app config TOML 的 `mcp_servers` 节；它支持 stdio command/args/env/cwd 和 streamable HTTP url/headers/oauth 等传输配置。
...
prompts 替代 templates。它们保存可复用的 system/developer 指令，不保存示例式模板。v1 只支持简单文本
提示词，不支持多 role prompt、多段 content parts、多模态 prompt 或变量模板。
...
`content` 映射为 `PromptContent { text: String }`。因为 fresh DB 仍是 pre-main baseline，后续实现按
baseline schema cleanup 处理，不为旧本地开发期 fresh DB 追加兼容 migration。
...
不属于项目、对话、provider、模型缓存或快捷键的全局 app 设置放在 app config 文件。当前 `app/ai-chat2`
使用同一个 `config.toml` 保存启动层路径、app 偏好、composer 默认偏好和 MCP source definitions：

- `[storage]`: `data_dir`
- `[app_settings]`: `AppSettingsPayload`，包括 language、theme、temporary hotkey、HTTP proxy 和默认项目
- `[chat_form]`: Composer 默认选择，包括 model provider/model id、reasoning selection 和 tool approval mode；provider/model 定义与 fetch cache 暂仍在 DB
- `[mcp_servers.<server_id>]`: MCP stdio / streamable HTTP server definition；`enabled = false` 的 server 不进入 runtime config layer

fresh database 不再创建 `app_settings` 表。历史对话、provider step 和 shortcut 仍在写入时保存需要重放的 settings snapshot。
...
- 2026-06-17 后续 baseline cleanup 已把全局 `app_settings` 和 ChatForm 默认选择偏好移到 `app/ai-chat2` 的 `config.toml`；DB 只保留 conversation/runtime 所需 snapshots 和索引。
