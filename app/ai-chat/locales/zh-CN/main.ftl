...
temporary-chat-save-default-name = 临时对话
temporary-chat-save-default-info = 由临时对话保存
...
field-required-capabilities = 所需能力
field-enabled = 启用
field-none = 无
field-on = 开
field-off = 关
...
capability-image-input = 图片输入
capability-image-generation = 图片生成
capability-file-input = 文件输入
capability-audio-input = 音频输入
capability-tool-calling = 工具调用
capability-hosted-web-search = 托管联网搜索
capability-remote-mcp = 远程 MCP
capability-reasoning = 推理
capability-structured-output = 结构化输出
capability-stateful-response-continuation = 有状态响应续写

dialog-add-folder-title = 新建文件夹
dialog-add-conversation-title = 新建会话
dialog-save-temporary-conversation-title = 保存临时对话
...
notify-save-temporary-conversation-success = 临时对话已保存
notify-save-temporary-conversation-failed = 保存临时对话失败
...
notify-shortcut-trigger-template-unavailable-title = 快捷键模板不可用
...
template-required-capabilities-none = 无所需能力
template-compatibility-compatible = 兼容
template-compatibility-incompatible = 不兼容
template-missing-capabilities = 缺失能力
chat-template-incompatible = 当前模型不兼容所选模板
...
shortcut-settings-description = 管理把选中文字、剪贴板或截图发送到临时对话的全局快捷键。
...
shortcut-status-template-unavailable = 模板不可用
...
shortcut-status-message-template-unavailable = 无法加载当前配置的模板。
shortcut-status-message-capability-mismatch = 缺失所需能力
...
shortcut-form-capability-warning = 可以保存该快捷键，但它不会在当前模型下运行
...
shortcut-mode-contextual-description = 携带对话上下文
shortcut-mode-single-description = 只发送本次内容
shortcut-mode-assistant-only-description = 只携带历史助手回复
send-content-selection-or-clipboard = 选中文字 / 剪贴板
shortcut-input-selection-or-clipboard-description = 优先选中文字，空时使用剪贴板
send-content-screenshot = 截图
shortcut-input-screenshot-description = 支持图片时发送图片，否则回退 OCR 文本
shortcut-screenshot-input-label = 截图
notify-shortcut-trigger-capability-mismatch-title = 快捷键能力不匹配
notify-shortcut-trigger-capability-mismatch-message = 缺失所需能力
