...
type OnUpdated = Rc<dyn Fn(ConversationTemplate, &mut App) + 'static>;
...
impl PromptEditorRow {
    fn new(role: Role, prompt: String, window: &mut Window, cx: &mut Context<PromptListForm>) -> Self {
        ...
        let template_fields = self
            ...
            .map(|row| {
                let child = match &row.value_state {
                    ...
                    TemplateFieldValueState::Select(select) => Select::new(select).into_any_element(),
                };
                field()
...
pub(crate) fn open_template_edit_dialog(
    ...
    let template_items = match template_inputs_by_adapter(&template.adapter, cx.global::<AiChatConfig>())
    {
        Ok(items) => items,
        Err(err) => {
    ...
    let adapter_subscription = window.subscribe(
        &adapter_input,
        cx,
        {
            ...
            move |_state, event: &SelectEvent<Vec<String>>, window, cx| {
                ...
                let next_form = cx.new(|cx| {
                    TemplateEditForm::new(template_items, &current_template, window, cx)
                });
                    ...
                    vec![
                        ...
                        Button::new("submit").primary().label("Submit").on_click({
                            ...
                            move |_, window, cx| {
                                ...
                                let description = {
                                    let value = description_input.read(cx).value().trim().to_string();
                                ...
                                let prompts = match prompt_form_input.read(cx).collect_prompts(cx) {
                                    Ok(prompts) => prompts,
                                    Err(err) => {
                                        window.push_notification(
                                            Notification::new()
