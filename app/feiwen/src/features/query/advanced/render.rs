...
use crate::features::query::QueryView;
use gpui::{
                ...
                v_flex()
                    ...
                    .children(
                        self.sorts
                            .iter()
                            .enumerate()
                            .map(|(ix, sort)| render_sort_row(ix, sort, cx)),
                    ),
            )
    }
}
...
fn render_condition(condition: &ConditionRow, cx: &mut Context<QueryView>) -> AnyElement {
    ...
    v_flex()
        ...
        .py_2()
        .border_1()
        .border_color(if condition.error.is_some() {
            cx.theme().danger
        } else {
            cx.theme().border
        })
                    ...
                    v_flex()
                        ...
                        .child(
                            h_flex()
                                ...
                                .child(
                                    Button::new(("condition-remove", condition_id))
                                        .xsmall()
                                        .ghost()
                                        .icon(IconName::Delete)
                                        .on_click(cx.listener(move |this, _, _, cx| {
...
fn render_relation_entity(relation_select: &RelationSelect) -> AnyElement {
    match relation_select {
        RelationSelect::Text(select) => Select::new(select).small().w_full().into_any_element(),
        RelationSelect::Number(select) => Select::new(select).small().w_full().into_any_element(),
        RelationSelect::Id(select) => Select::new(select).small().w_full().into_any_element(),
        RelationSelect::Bool(select) => Select::new(select).small().w_full().into_any_element(),
        RelationSelect::Tags(select) => Select::new(select).small().w_full().into_any_element(),
        RelationSelect::Author(select) => Select::new(select).small().w_full().into_any_element(),
    }
}
...
fn render_number_value(value: &NumberValue) -> AnyElement {
    match value {
        NumberValue::Single(input) => NumberInput::new(input).small().w_full().into_any_element(),
...
fn render_id_value(value: &IdValue) -> AnyElement {
    match value {
        ...
        IdValue::Number(input) => NumberInput::new(input).small().w_full().into_any_element(),
    ...
    v_flex()
        ...
        .child(
            h_flex()
                ...
                .py_2()
                .border_1()
                .border_color(if sort.error.is_some() {
                    cx.theme().danger
                } else {
                    cx.theme().border
                })
                ...
                .child(
                    v_flex()
                        ...
                        .child(Select::new(&sort.direction_select).small().w_full()),
                )
                .child(
                    Button::new(("sort-remove", sort_id))
                        .xsmall()
                        .ghost()
                        .icon(IconName::Delete)
                        .on_click(cx.listener(move |this, _, _, cx| {
