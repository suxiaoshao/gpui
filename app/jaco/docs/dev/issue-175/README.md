...
`codex/175-jaco-shortcut-temporary-window`，基线为 `origin/main@3cb5687`。ChatForm/ControlSlot、typed catalog、
pure draft handle、attachment single owner 和 submit resolver 已按后续设计落地；快捷键与提示词设置也已迁移到
caller-owned subscriptions。代码级验证已通过，快捷键/临时窗口的实际桌面 UI 验收按用户要求跳过。provider/MCP
也已迁移到同一 adapter 生命周期；桌面生命周期验收继续由
[state-ownership-sync-plan.md](state-ownership-sync-plan.md) 跟踪。
后续手动运行还复现过 picker/temporary list 的 `ListState ... already being updated` 和
`RefCell already borrowed`，因此 WP-110 的 GPUI 生命周期验证仍是未完成的后续 gate。
...
- 原 issue 实施阶段不迁移到 `gpui-store`、不修改 `gpui-form` crates；后续状态所有权计划允许在保持两者解耦的
  前提下让 `gpui-form` 只拥有 draft，并使用 app-created component state + adapter-returned subscriptions；catalog 使用
  `gpui-store` 现有 snapshot/backend 能力且不写入 form。
...
- [state-ownership-sync-plan.md](state-ownership-sync-plan.md)：form draft、committed store、catalog projection
  和 controller 重复 owner 的实施入口及剩余迁移。
...
### D-04：form value、component config、interaction 三通道（已实施）

form 是唯一用户 draft owner；provider/project catalog 和 disabled/capability/options 是 app-owned component
config；focus/open/query/highlight/scroll/task 是 component-owned interaction。component 中的 text/selected 只是
form value mirror。options 更新不 replace/rebase form，submit 不读取 component state。旧 state-owning
`FormComponentBinding` 已删除，由 app-created state + component-specific bind functions + caller-owned
`SubscriptionSet` 替代。

### D-05：RunSettings resolver 纯计算，controller 只执行 commands

model confirm/approval 是 form command；catalog reload 只更新 options/config；reasoning validity 与 fallback 由
无副作用 resolver 计算；picker互斥属于 interaction state。Conversation安装 typed form event persistence adapter；
Shortcut不安装。

### D-06：Project slot统一布局，owner不变

Project picker的trigger/bar/footer视觉移动到ChatForm project control/render。当前 project selection 由
`NewConversationPage` 的 canonical selection state 持有；label/icon/rows 由 selection + typed catalog 纯派生，
`ProjectControlState` 只持有 picker interaction。default project config、add-folder task、catalog reload 和 skill catalog refresh 由
页面/服务显式处理，不能把 default preference 或 projection 作为当前 project 的第二个 owner。Hidden project slot使
普通会话/临时/Shortcut不产生bar或bottom padding。
...
快捷键完成回调先通过 `cx.defer` 离开 `GlobalHotkeyState` 的更新作用域，再进入 temporary-window lifecycle，避免
在 hotkey global update 内嵌套 window/entity 更新导致 `RefCell already borrowed`。
...
- `app/jaco/docs/dev/issue-175/state-ownership-sync-plan.md`：后续所有权/同步架构计划。
...
- 原 issue 实施阶段未修改 `crates/gpui-form*`、`crates/jaco-core/*`、`crates/jaco-db/*`；后续计划允许
  `gpui-form`/macro/adapter 做 breaking ownership refactor，但不新增 `gpui-form`↔`gpui-store` 依赖。
...
| gpui-form group/value fields | Refactor to pure draft core | form draft 继续复用；不增加 selection/rebase/conflict 或隐式 binding |
| `ProjectControlState` / run-settings control caches | Projection only | 删除其业务 setter 和 submit/validation 事实读取 |
| revision-only project/provider entities | Replace | 改为 app-owned typed catalog snapshot，revision 只作 metadata |
| 手写 form↔store observer mirror | Delete | catalog snapshot 只投影 component config；domain value 只显式 replace/commit |
...
依赖：`WP-10 -> WP-20 -> WP-30 -> WP-40 -> WP-50 -> WP-60`；后续状态模型依赖
`WP-70 -> WP-80 -> WP-90 -> WP-100 -> WP-110`。WP-10 至 WP-50 已实施，WP-60 已完成代码级回归并已构建
本地 Jaco bundle；实际 Computer Use/窗口交互验收按用户要求不执行。
...
已完成格式化、编译、clippy、测试目标编译、focused policy/shortcut/hotkey 测试以及
`cargo run -p xtask -- bundle jaco`。bundle 已生成；实际 Computer Use、快捷键 popup 路由和窗口交互验收明确跳过，
后续可单独作为桌面验收任务执行。bundle 过程中的 `actool` Liquid Glass 图标注入警告由 xtask 降级处理，不影响普通图标
和 bundle 产物。

### WP-70 至 WP-110：状态所有权与外部同步（代码实施完成）

具体依赖、文件、API、错误语义、测试和 done condition 统一见
[state-ownership-sync-plan.md](state-ownership-sync-plan.md)：

- `FORM-10..40`：pure draft、typed handle、generic events 和 adapter subscriptions 已落地；workspace 旧
  state-owning binding 已删除。
- `WP-80`：provider/project/shortcut typed `SharedStore` catalog 已落地；
- `WP-90`：ChatInput/RunSettings/Shortcut 主路径已完成 options/config 与 form value 分流，attachments 已收敛；
- `WP-100`：普通会话 fallback、快捷键 strict selection、reasoning/attachment capability 已由 pure resolver 统一；
- `WP-110`：保留桌面级 GPUI nested-update/reentrancy 验收。
...
| 状态 | form 是 draft owner；store 是 catalog owner；app 是 component config owner；component 是 interaction owner |
...
上述命令均已通过；状态所有权计划中的 `FORM-10..40`、`WP-80`、`WP-90`、`WP-100` 也已完成对应的
crate/app 回归测试。`cargo test --no-run` 只用于编译测试目标，不会启动 GPUI 窗口；本轮已额外构建 Jaco bundle，
但按用户要求未运行实际桌面 UI 或 Computer Use 验收。`WP-110` 的桌面级生命周期验收仍单独保留为后续 gate。
