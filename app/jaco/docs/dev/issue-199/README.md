...
- 状态：`Done`；`JACO-199-01`–`JACO-199-04` 均已完成
...
- 本地 ID 范围：历史 `E/D/F/L/ST/R/T-400..499`、`WP-400..499` 与
  `E/D/F/L/ST/R/T-600..699`、`WP-600..699`；已完成轮次
  `E/D/F/L/ST/ERR/R/T-1200..1299`、`WP-1200..1209`；当前轮次
  `E/D/F/L/ST/ERR/R/T-1300..1339`、`WP-1300..1305`
...
| `JACO-199-03` | 迁移到最终 Form path、event、binding 与 FormVersion 契约 | `Done`；362 tests、Clippy 与 workspace gate 通过；实际 UI 操作测试未执行 | [form-breaking-api-remigration-plan.md](form-breaking-api-remigration-plan.md) | `WP-1200`–`WP-1204` | Form `C-900`–`C-904` 已达 `consumer-complete` | 以专题完成证据为准 |
| `JACO-199-04` | Conversation 提交、active-run、停止与迟到 completion 的私有 Transition | `Done`；聚焦 Conversation 42 tests、Jaco 全量 369 tests 与严格 Clippy 通过；实际 UI、打包与跨平台 CI 未执行 | [conversation-runtime-transition-plan.md](conversation-runtime-transition-plan.md) | `WP-1300`–`WP-1305` | MCP runtime 已移交 [#201](https://github.com/suxiaoshao/gpui/issues/201)，不改 DB schema 或 Store | `99a073a` |
