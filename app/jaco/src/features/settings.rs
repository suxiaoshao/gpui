    ...
    match exists_settings {
        Some(window) => {
            ...
            match window.update(cx, |root, window, cx| {
                        ...
                        match window.native_window_handle() {
                            ...
                            Err(err) => {
                                event!(
                                    Level::ERROR,
                                    error = ?err,
                                    "get jaco settings window handle failed"
                                );
                            }
                        }
                    }
                ...
                Ok(_) => {}
                Err(err) => {
                    event!(Level::ERROR, error = ?err, "activate jaco settings window failed");
                }
            };
...
fn inner_open_settings_window(selected_page: Option<SettingsPageKey>, cx: &mut App) {
    ...
    match cx.open_window(
        ...
        Err(err) => {
            event!(Level::ERROR, error = ?err, "open jaco settings window failed");
        }
    };
}
...
fn schedule_settings_window_reveal(native_window: NativeWindowHandle, cx: &mut App) {
    cx.defer(move |_| {
        if let Err(err) = native_window.show() {
            event!(Level::ERROR, error = ?err, "show jaco settings window failed");
        }
    });
}
...
fn settings_page_specs_for_i18n(i18n: &I18n) -> [SettingsPageSpec; 9] {
    ...
    [
        ...
        SettingsPageSpec::new(
            ...
            settings_search_text(
                [page_usage.as_str()],
                "usage token tokens statistics analytics request requests provider model cache 使用 用量 统计 令牌 请求 提供商 模型 缓存 shiyong yongliang tongji lingpai qingqiu tigongshang moxing huancun sy yl tj lp qq tgs mx hc",
            ),
        ),
        SettingsPageSpec::new(
            SettingsPageKey::Projects,
