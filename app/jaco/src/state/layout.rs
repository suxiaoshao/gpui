...
#[derive(Clone)]
pub(crate) struct LayoutStateStore(Entity<JacoLayoutState>);

impl LayoutStateStore {
    pub(crate) fn entity(&self) -> Entity<JacoLayoutState> {
        self.0.clone()
    }
}
...
pub(crate) struct JacoLayoutState {
...
impl JacoLayoutState {
    ...
    pub(crate) fn save_now(&self) -> JacoResult<()> {
        let path = Self::path()?;
    ...
    fn schedule_save(&mut self, cx: &mut Context<Self>) {
        ...
        self.save_task = Some(cx.spawn(async move |_, _cx| {
            ...
            let path = match path {
                Ok(path) => path,
                Err(err) => {
                    event!(Level::ERROR, error = ?err, "resolve jaco state path failed");
                    return;
                }
            };

            if let Err(err) = write_persisted_to_path(&path, &snapshot) {
                event!(Level::ERROR, error = ?err, "save jaco state.toml failed");
            }
        }));
    }

    fn load_or_create() -> JacoResult<Self> {
        let path = Self::path()?;
...
pub(crate) fn restored_window_placement(
    ...
    let persisted = if cx.has_global::<LayoutStateStore>() {
        ...
        match JacoLayoutState::path().and_then(|path| load_or_create_from_path(&path)) {
            Ok(persisted) => persisted,
            Err(err) => {
                event!(Level::ERROR, error = ?err, "load jaco layout state for window placement failed");
...
pub(crate) fn save_now(cx: &App) {
    ...
    if let Err(err) = state.read(cx).save_now() {
        event!(Level::ERROR, error = ?err, "save jaco state.toml on quit failed");
    }
}
...
fn load_or_create_from_path(path: &Path) -> JacoResult<PersistedLayoutState> {
    match fs::read_to_string(path) {
        Ok(source) => match toml::from_str::<PersistedLayoutState>(&source) {
            ...
            Err(err) => {
                event!(Level::ERROR, error = ?err, "parse jaco state.toml failed");
...
fn write_persisted_to_path(path: &Path, state: &PersistedLayoutState) -> JacoResult<()> {
...
mod tests {
    use super::{
        JacoLayoutState, PersistedLayoutState, PersistedWindowBounds, PersistedWindowMode,
    ...
    fn sidebar_width_clamps_loaded_layout_values() {
        let below_min = JacoLayoutState::new(PersistedLayoutState {
            version: STATE_VERSION,
            sidebar_width: f32::from(SIDEBAR_MIN_WIDTH) - 1.,
            ..PersistedLayoutState::default()
        });
        let above_max = JacoLayoutState::new(PersistedLayoutState {
            version: STATE_VERSION,
            sidebar_width: f32::from(SIDEBAR_MAX_WIDTH) + 1.,
            ..PersistedLayoutState::default()
        });
        let normal = JacoLayoutState::new(PersistedLayoutState {
            version: STATE_VERSION,
            sidebar_width: f32::from(SIDEBAR_DEFAULT_WIDTH),
            ..PersistedLayoutState::default()
        });
