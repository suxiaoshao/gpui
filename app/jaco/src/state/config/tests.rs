use super::{
    AppSettingsConfig, ChatFormConfig, ChatFormModelConfig, JacoAppSettings, JacoConfig,
...
use jaco_agent::McpServerTransport;
use jaco_core::{
...
fn toml_config_stores_storage_and_app_settings() {
    let config = JacoConfig {
        storage: StorageConfig {
            data_dir: Some(PathBuf::from("/tmp/jaco")),
        },
    ...
    assert_eq!(toml::from_str::<JacoConfig>(&serialized).unwrap(), config);
}

#[test]
fn toml_config_ignores_unknown_fields_for_compatibility() {
    let config = toml::from_str::<JacoConfig>(
        r#"
...
fn new_toml_config_writes_default_app_settings() {
    ...
    let config = JacoConfig::load_or_create_from_path(&path).expect("create config");

    assert_eq!(config, JacoConfig::default());
...
fn config_directory_override_ignores_empty_env_value() {
    ...
    assert_eq!(
        override_dir_from_value(Some(OsString::from("/tmp/jaco-config"))),
        Some(PathBuf::from("/tmp/jaco-config"))
    );
}
...
fn malformed_toml_config_preserves_file_and_returns_diagnostic_default() {
    ...
    let config = JacoConfig::load_or_create_from_path(&path).expect("load fallback config");
...
fn mcp_server_crud_helpers_persist_config_toml(cx: &mut TestAppContext) {
    ...
    let stored =
        JacoConfig::load_or_create_from_path(&path).expect("load stored config after create");
    ...
    let stored =
        JacoConfig::load_or_create_from_path(&path).expect("load stored config after edit");
    let filesystem = stored
        .mcp_servers
        .get("filesystem")
        .expect("filesystem server is stored");
    ...
    let stored =
        JacoConfig::load_or_create_from_path(&path).expect("load stored config after delete");
    assert!(stored.mcp_servers.is_empty());
}

#[test]
fn app_settings_expose_typed_config_preferences() {
    let settings = JacoAppSettings::new(AppSettingsPayload {
        language: AppLanguage::Chinese,
        theme: AppThemeSettings {
            mode: AppThemeMode::Light,
            ..Default::default()
...
transport = "streamable_http"
url = "https://docs.example.com/mcp"
env_vars = ["JACO_HTTP_STDIO_ENV_SHOULD_NOT_BE_READ"]
cwd = "/tmp/stdio-only"
...
fn mcp_config_rejects_reserved_headers_and_oauth_authorization_header() {
    let reserved = toml::from_str::<JacoConfig>(
        r#"
[mcp_servers.bad]
transport = "streamable_http"
url = "https://example.com/mcp"
...
"#,
    ...
    let authorization = toml::from_str::<JacoConfig>(
        r#"
[mcp_servers.bad]
transport = "streamable_http"
url = "https://example.com/mcp"
...
"#,
    ...
    let bearer_token = toml::from_str::<JacoConfig>(
        r#"
[mcp_servers.bad]
transport = "streamable_http"
url = "https://example.com/mcp"
...
fn noop_app_settings_update_does_not_persist(cx: &mut TestAppContext) {
    ...
    let config = JacoConfig::with_app_settings_for_test(path.clone(), payload.clone());
    config.save_for_test().expect("save config");
