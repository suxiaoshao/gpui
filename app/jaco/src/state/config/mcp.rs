...
impl McpServerTomlConfig {
    pub(crate) fn to_agent_config(&self, server_id: &str) -> JacoResult<McpServerConfig> {
        self.validate(server_id)?;
        let (transport, env, cwd) = match self.transport {
            McpTransportKind::Stdio => {
                let command = self.command.clone().ok_or_else(|| {
                    JacoError::Config(format!("mcp server `{server_id}` is missing command"))
                })?;
                (
            ...
            McpTransportKind::StreamableHttp => {
                let url = self.url.clone().ok_or_else(|| {
                    JacoError::Config(format!("mcp server `{server_id}` is missing url"))
                })?;
                (
    ...
    pub(crate) fn validate(&self, server_id: &str) -> JacoResult<()> {
    ...
    fn resolved_env(&self, server_id: &str) -> JacoResult<BTreeMap<String, String>> {
    ...
    fn resolved_headers(&self, server_id: &str) -> JacoResult<BTreeMap<String, String>> {
...
impl McpOAuthTomlConfig {
    fn validate(&self, server_id: &str) -> JacoResult<()> {
        match self {
...
fn validate_timeout(server_id: &str, field: &str, value: Option<u64>) -> JacoResult<()> {
...
fn validate_tool_filter(server_id: &str, field: &str, tools: Option<&[String]>) -> JacoResult<()> {
...
fn validate_raw_tool_name(server_id: &str, field: &str, tool_name: &str) -> JacoResult<()> {
...
fn validate_env_var_name(server_id: &str, env_var: &str) -> JacoResult<()> {
...
fn validate_http_url(server_id: &str, value: &str) -> JacoResult<()> {
