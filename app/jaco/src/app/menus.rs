    ...
    match platform_ext::app::set_windows_menu_from_main_menu_index(WINDOW_MENU_INDEX) {
        ...
        Err(err) => {
            event!(
                ...
                error = ?err,
                index = WINDOW_MENU_INDEX,
                "failed to register localized jaco window menu with NSApp"
            );
        }
    }
}
...
mod tests {
    ...
    fn builds_expected_top_level_menus() {
        ...
        assert_eq!(menu_names(&app_menus(&i18n)), vec!["Jaco", "Window"]);
    }
    ...
    fn builds_expected_top_level_menus_for_chinese() {
        ...
        assert_eq!(menu_names(&app_menus(&i18n)), vec!["Jaco", "窗口"]);
    }
