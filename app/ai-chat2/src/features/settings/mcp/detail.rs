...
pub(super) fn render_server_info(
    info: ai_chat_agent::McpServerInfoSnapshot,
    cx: &mut App,
) -> AnyElement {
    let i18n = cx.global::<I18n>();
...
pub(super) fn render_tools(tools: Vec<ai_chat_agent::McpToolSnapshot>, cx: &mut App) -> AnyElement {
...
fn render_tool(tool: ai_chat_agent::McpToolSnapshot, cx: &mut App) -> AnyElement {
    let name = tool.name;
    let title = tool
        .title
