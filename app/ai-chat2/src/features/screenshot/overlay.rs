...
actions!(ai_chat2_screenshot_overlay, [CancelScreenshotSelection]);

const CONTEXT: &str = "AiChat2ScreenshotOverlay";
...
pub(crate) fn open(shortcut: ShortcutRecord, cx: &mut App) -> Result<(), CaptureError> {
    ...
    event!(
        ...
        "opening ai-chat2 screenshot overlay"
    );
    ...
    match handle {
        ...
        Err(err) => Err(CaptureError::SystemFailure(err.to_string())),
    }
}
...
impl ScreenshotOverlayView {
    fn new(display: CaptureDisplay, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let subscriptions = vec![cx.observe_window_activation(window, |this, window, _cx| {
            if window.is_window_active()
                && let Err(err) = window.set_crosshair_cursor_rect()
            {
                event!(
                    Level::ERROR,
                    display_id = this.display.id_hint,
                    error = ?err,
                    "failed to set screenshot overlay cursor"
                );
            }
        })];
        if let Err(err) = window.set_crosshair_cursor_rect() {
            let display_id_hint = display.id_hint;
            event!(
                Level::ERROR,
                display_id = display_id_hint,
                error = ?err,
                "failed to set screenshot overlay cursor"
            );
        }
    ...
    fn handle_mouse_up(
        ...
        let shortcut = cx
            ...
            .session
            .as_ref()
            .map(|session| session.shortcut.clone());
        ...
        if let Err(err) = window.clear_cursor_rects() {
            event!(
                ...
                error = ?err,
                "failed to clear screenshot overlay cursor"
            );
        }
        window.remove_window();
            ...
            match captured {
                Ok(image) => {
                    cx.update_global::<GlobalHotkeyState, _>(|hotkeys, cx| {
                        hotkeys.process_captured_screenshot(shortcut.clone(), image, cx);
                    });
                }
                Err(err) => {
                    cx.update_global::<GlobalHotkeyState, _>(|hotkeys, cx| {
