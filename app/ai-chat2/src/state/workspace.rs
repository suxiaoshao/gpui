...
use ai_chat_core::{ConversationId, ConversationStatus, ProjectId, ProjectKind};
use ai_chat_db::{ConversationRecord, ProjectRecord};
use gpui::{App, AppContext, Context, Entity, Global, SharedString, Subscription};

use crate::{database, state::projects};

#[derive(Clone)]
pub(crate) struct WorkspaceStoreGlobal(Entity<AiChat2WorkspaceStore>);

impl WorkspaceStoreGlobal {
    pub(crate) fn entity(&self) -> Entity<AiChat2WorkspaceStore> {
        self.0.clone()
    }
}
...
pub(crate) struct AiChat2WorkspaceStore {
    route: HomeRoute,
...
impl AiChat2WorkspaceStore {
    ...
    pub(crate) fn search_conversations(
        ...
        limit: usize,
        cx: &App,
    ) -> ai_chat_db::Result<Vec<SidebarSearchResult>> {
        let repository = database::repository(cx);
...
pub(crate) fn init(cx: &mut App) {
    ...
    let store = cx.new(|cx| AiChat2WorkspaceStore::new(project_catalog, cx));
    cx.set_global(WorkspaceStoreGlobal(store));
}

pub(crate) fn workspace(cx: &App) -> Entity<AiChat2WorkspaceStore> {
