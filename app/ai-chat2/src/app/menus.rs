    ...
    match platform_ext::app::set_windows_menu_from_main_menu_index(WINDOW_MENU_INDEX) {
        Ok(()) => {
            event!(
                ...
                "registered localized ai-chat2 window menu with NSApp"
            );
        }
        Err(err) => {
            event!(
                ...
                error = ?err,
                index = WINDOW_MENU_INDEX,
                "failed to register localized ai-chat2 window menu with NSApp"
            );
        }
    }
}
...
mod tests {
    ...
    fn builds_expected_top_level_menus() {
        ...
        assert_eq!(menu_names(&app_menus(&i18n)), vec!["AI Chat 2", "Window"]);
    }
    ...
    fn builds_expected_top_level_menus_for_chinese() {
        ...
        assert_eq!(menu_names(&app_menus(&i18n)), vec!["AI 对话 2", "窗口"]);
    }
    ...
    fn builds_expected_app_menu_items() {
        ...
        assert_eq!(
            item_names,
            vec![
                "About AI Chat 2".to_string(),
                format!("Version (v{})", env!("CARGO_PKG_VERSION")),
                "---".to_string(),
                "Open AI Chat 2".to_string(),
                "Open Temporary Conversation".to_string(),
                "Settings".to_string(),
                "---".to_string(),
                "Services".to_string(),
                "---".to_string(),
                "Hide AI Chat 2".to_string(),
                "Hide Others".to_string(),
                "Show All".to_string(),
                "---".to_string(),
                "Quit AI Chat 2".to_string(),
            ]
        );
