...
- 单实例获取或监听初始化错误由现有 `StartupView` 恢复页承接，显示具体错误与重试、定位目录、退出入口。重试在后台重新获取所有权；成功后加载配置，若已有进程确认唤醒则直接退出。取得所有权前仅读取已有布局，不删除或保存布局、不写入共享日志、不发布临时窗口配置、不启动 Tray/Pi；设置菜单不能绕过恢复页，也不提供与这类错误无关的配置重置。
- 恢复回归覆盖端点发布失败后重试、已有实例无法唤醒时仍保持独占、错误页不加载配置及设置菜单不能绕过恢复。macOS 隔离目录实测已确认：只读目录能显示恢复页；恢复权限后重试进入既有配置修复页，再修正配置并重读可进入主页；锁被占用时退出恢复页，配置、无效布局文件和共享日志字节均不变。受影响测试与 Clippy 通过，macOS 应用包已重建。
