...
分叉、显示根和叶子是结构边界。当前执行位置、所选节点、预览定位点，以及带标签、失败和压缩的可见节点均保留。内容过滤本身不显示的定位点映射到最近可见祖先；不额外恢复被过滤的记录。当前分支同样允许折叠，普通用户消息不会仅因角色而成为保留节点。
...
| D3 | 默认折叠 | 至少隐藏 3 个节点才折叠；保留显示树的分叉、根、叶、选择/预览/执行定位点，以及带标签、失败和压缩的可见节点；当前分支也允许折叠 |
...
- `cargo test -p gupi --locked` 通过 49 项；补充文字避让回归后，`cargo test -p gupi history --locked` 通过 16 项。新增覆盖折叠展开往返、计数与保留节点、选中后禁止收起、快照追加保留展开状态、1 万节点单链、2000 个同父分支、嵌套分叉、多个显示根、穿屏边、缩放锚点、曲线命中和标签避让。
...
- macOS 原生验证使用隔离模拟 Pi RPC 和临时会话：26 节点单链折叠为两端与数字 24；在线条旁 9px 点击可展开，端点菜单可收回整段，展开保持端点位置。拖动折叠线只平移，不误展开；停留后出现“展开 24 个节点”提示。
