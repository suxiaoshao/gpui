...
- 草稿加载与目录扫描继续由 ConversationState 协调：启动先加载非空草稿，再把已知 cwd 交给扫描；草稿加载单独表达，不伪装成目录读取进度。
...
- 模型弹层首次打开即加载尚未读取的选项，加载中不重复请求，成功的空列表也视为已加载。新空白会话为读取模型启动连接时，使用连接握手取得实际模型状态，再分别读取模型与思考档位；不读取 entries、统计或 fork 信息。模型选项加载复用当前会话，首次发送可复用该连接。
- 切换工作目录先查找目标目录已有的空白会话，直接选回原会话及其连接、模型设置和加载任务，不重新启动 Pi。仅复用无会话文件、输入、历史或实时消息，且没有正在进行的对话、核心读取、会话操作或待处理交互的会话；模型选项加载不阻止复用。目标目录没有可复用会话时沿用原有创建规则；尚未连接且已有输入的草稿仍原地变更目录，保留输入。
...
- 普通 prompt 提交由每个会话独立持有提交 task：首次连接和重连期间，直到 Pi 连接及初始会话状态就绪，主输入框只读并保留原文，禁止提交和再次刷新，不暂存待连接后自动发送的请求；就绪后，从用户点击提交到 RPC `prompt` 返回期间保留原文，输入框只读但可复制，发送按钮禁用并显示 loading，键盘重复提交被阻止；仅在成功响应后清空对应输入并恢复编辑，不等待模型完成。失败时保留原文并显示通知，恢复提交以便重试；不维护失败输入恢复字段或队列。扩展 `select` / `confirm` / `input` / `editor` 的临时输入区仍可独立回答，不与普通 prompt 提交状态混用，不同会话互不影响。
- 先保证一致性再安排并发。读取结果绑定连接实例、会话 binding 和相关内容版本；模型能力结果还须对应实际模型。旧读取不能覆盖更新的 Pi 事件，不用附加数据完成去清除其他操作的运行状态。
- 扩展待处理交互、标题、状态文字和 widget 属于当前 Pi 进程。断线、连接失效或主动移除连接时统一清除，避免重连后沿用旧进程的界面；清理不修改历史、会话文件或普通输入草稿。成功 fork 转交同一连接时一并转交扩展界面，不触发这项清理。
...
- 重命名和 fork 的 `SessionCommand` 只持有任务。插件 `set_editor_text` 始终直接更新来源会话输入，不为 fork 暂存或提升优先级；按 Pi TUI 的顺序，fork 成功后用 RPC 返回的所选历史消息回填新会话，取消或失败不执行这次回填。成功后的核心数据读取使用既有加载与重试流程，读取失败保留已完成的 fork、连接和返回文字，不重复 fork；成功时即请求目录刷新。模型修改的已确认失败和无法确认分别表达，后者继续阻止发送。
...
- 重命名弹窗只读取目录标题，不恢复会话或启动 Pi。确认时由状态层检查目标会话操作状态：已有实例必须连接就绪后才通过 RPC 改名；未连接的历史文件通过正常连接流程核对身份并启动 Pi，复用扩展事件消费和历史同步后调用 set_session_name；文件格式及写入由 Pi 负责。成功后更新内存标题并请求目录扫描，失败通知；确认未被接收时保留弹窗和输入。
...
- 受控 Pi 进程回归覆盖附加请求卡住/失败不阻塞核心与发送、连接及初始读取期间拒绝提交且就绪后不自动发送、局部重试不重读历史、模型命令失败后读回实际值、慢档位请求不阻塞已确认模型的发送，以及旧历史快照不能覆盖读取期间的新事件。
- 新空白会话回归确认：首次模型读取只使用一次连接握手及模型/思考请求，不产生历史、统计、fork 请求或目录扫描；显式刷新模型与目录互不触发，模型连接启动期间禁止提交，就绪后主动提交只发送一次。弹层回归确认首次打开自动加载、加载中去重，以及成功的空列表不被误判为未加载。
