...
- 重命名弹窗只读取目录标题，不恢复会话或启动 Pi。确认时由状态层检查目标会话操作状态：已有实例必须连接就绪后才通过 RPC 改名；未连接的历史文件核对身份和 v3 格式后，仅追加一条 session_info，沿用文件最后条目的 id 作为 parentId。成功后更新内存标题并请求目录扫描，失败通知；确认未被接收时保留弹窗和输入。
