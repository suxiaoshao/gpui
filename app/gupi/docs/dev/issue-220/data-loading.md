...
- 普通 prompt 提交由每个会话独立持有提交 task：从用户点击提交（包括等待连接建立）到 RPC `prompt` 返回期间保留原文，输入框只读但可复制，发送按钮禁用并显示 loading，键盘重复提交被阻止；仅在成功响应后清空对应输入并恢复编辑，不等待模型完成。失败时保留原文并显示通知，恢复提交以便重试；不维护失败输入恢复字段或队列。扩展 `select` / `confirm` / `input` / `editor` 的临时输入区仍可独立回答，不与普通 prompt 提交状态混用，不同会话互不影响。
