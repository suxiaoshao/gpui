...
| 内层折叠 | 与第一级独立。两个 assistant 正文之间的活动段只有一个工具或思考项时直接显示该项，不增加二级分组折叠；至少两个活动项时才显示二级分组，默认收起，包括当前运行组。思考详情继续原位折叠；单个工具点击后打开详情 Dialog，分区查看和复制，不再内层展开长正文，见[详情方案](../issue-238/README.md)。 |
