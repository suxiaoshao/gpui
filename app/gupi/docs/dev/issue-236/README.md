...
- **R-11：** 相同有效语言不重建 Fluent/global locale；相同模式/主题不重复应用或刷新窗口，系统强调色变化仍可强制更新。菜单与托盘文字按语言变化更新；原生菜单还会在快捷键实际变化后重建，确保首次注册、修改、清除和恢复绑定同步到 macOS 菜单快捷键，托盘不随绑定变化刷新。正常主题/语言预览与取消仍通过已有配置流恢复。
...
- 原生菜单绑定同步回归：`cargo test -p gupi state::keybindings::tests --quiet` 3 项通过，覆盖先创建菜单再注册绑定、修改、清除、恢复和重复配置去重。测试平台只验证菜单重建调用，不提供 macOS 原生快捷键渲染验证。
