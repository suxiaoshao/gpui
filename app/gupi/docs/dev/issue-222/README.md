...
状态：等待正式组件版本。按用户 2026-09-19 决定，等待 Questionnaire #2878、InputGroup #3042、原子内联标签 #3113 全部进入兼容的 gpui-kit 正式版本后，再统一恢复本部分 UI 接入与输入框改造。三个 PR 已合并，但 v0.6.4 尚未包含 Questionnaire 和内联标签。临时插件和隔离启动脚本保留，完整插件 UI 验收尚未完成。
...
本体验环境不修改生产界面、不安装插件、不修改用户 Pi 配置。插件/包、Skill 和提示词管理已由 [#231 设置计划](../issue-231/README.md)接入；不属于本节扩展交互 UI 的暂停范围。任意 TUI 组件与同文件树节点续聊继续保持现有边界。跨阶段遗留项统一见[主 Issue 索引](../../../../../docs/dev/issue-217/follow-ups.md)。

## 当前结论与恢复条件

最新依赖状态、33 个 RPC 命令和事件/扩展 UI 的接入盘点统一见[总待处理文档](../../../../../docs/dev/issue-217/follow-ups.md)。组件发布只解除客户端控件依赖，不补充 Pi 的组合问卷协议；以下早期调研结论仍按实际 RPC 契约使用。
...
恢复工作时，先核对上游 question 组件的实际 API，以及当前 gpui-kit 依赖是否已包含它，再决定如何映射 RPC 的 select/confirm/input 等消息。组件提供的视觉能力不改变 Pi 协议，不能由此推断 RPC 新增了组合问卷或 custom 支持。0.6.6 正式包尚未包含 Questionnaire，恢复条件由 #243 统一维护。
