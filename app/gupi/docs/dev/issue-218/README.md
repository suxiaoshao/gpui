...
已应用配置与设置草稿各自的 `Entity<PiProbeController>` 持有 `refresh::Operation`；精确转移和任务归属见运行时 L-112。输入为 Pi/插件页面首次需要时检查的已应用 pi_command 或设置页显式检测的草稿命令；输出为解析后的命令路径与版本字符串。已应用命令结果按命令复用；Pi/插件页面需要新命令结果时，通过 Cancel 丢弃旧任务并重新检测。重开设置不强制检测，主题/语言变化不触发探测。应用启动不运行版本探测，普通和临时会话从已保存配置直接创建，在会话连接阶段查找可执行文件并启动 RPC。
...
配置读取结果 Configured -> 主会话界面
创建/恢复会话 -> 查找命令并启动 RPC -> 会话内显示连接状态或错误
...
| R-105 数据决定界面 | T-105，启动 controller 测试 | config 缺失进入 welcome 且保存前不创建文件；手动提供有效配置跳过 welcome；保存后退出重启直接进入主界面；损坏/权限失败进入恢复；启动时删除配置重新设置；配置失败进入恢复页，Pi 检测不影响主界面与本地历史，连接失败归会话 |
