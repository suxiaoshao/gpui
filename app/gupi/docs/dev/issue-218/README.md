...
已应用配置与设置草稿各自的 `Entity<PiProbeController>` 持有 `refresh::Operation`；精确转移和任务归属见运行时 L-112。输入为打开设置时检查的已应用 pi_command 或设置页显式检测的草稿命令；输出为解析后的命令路径与版本字符串。设置打开时，配置路径变化会通过 Cancel 丢弃旧任务并重新检测；主题/语言变化不触发探测。应用启动不运行版本探测，普通和临时会话从已保存配置直接创建，在会话连接阶段查找可执行文件并启动 RPC。
