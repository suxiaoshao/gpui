...
- Enter 发送或 steer，Alt+Enter 提交 follow-up，Shift+Enter / Cmd+Enter 换行。每个会话保留独立草稿；项目选择位于输入框工具栏，模型和思考选项来自 Pi；上下文圆环与下方累计 Token 用量各自提供独立详情。
...
- “会话历史”标题行提供图标 Toggle Group：默认简略模式只显示用户和助手消息，详细模式还显示工具结果和摘要等节点；两种模式均沿用 Pi 默认规则过滤设置和无正文的助手记录，不自动收起过程节点。悬停节点可查看时间。预览其他分支保留实际执行位置和草稿，暂停普通发送，可返回当前分支。支持的用户消息可通过原生 fork 另开会话，保留来源和原草稿。
