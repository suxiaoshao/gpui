...
settings-system-prompt-help = SYSTEM.md 替换默认系统提示词；APPEND_SYSTEM.md 追加个人指令。保存后由你手动刷新已有会话。
settings-package-counts = 扩展 { $extensions } · Skill { $skills } · 模板 { $prompts } · 主题 { $themes }
settings-skill-search = 搜索 Skill 名称或说明
settings-about-gupi = Gupi 版本
settings-about-pi = Pi 版本
settings-about-path = Pi 实际路径
settings-about-status = Pi 检查状态
settings-about-unavailable = 尚未取得
settings-editor-discard = 放弃未保存的正文修改吗？
settings-resource-close = 返回列表
settings-package-confirm-remove = 从个人 Pi 中移除此包？本地包只取消注册，保留来源目录。
settings-package-heading = Pi 包
settings-extension-heading = 扩展
settings-template-heading = 命令模板
settings-system-heading = 系统提示词
settings-package-open = 打开所属包目录
