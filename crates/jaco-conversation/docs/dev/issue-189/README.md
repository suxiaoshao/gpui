# jaco-conversation：hydrate Agent message request usage
...
- 消费 root IDs：`C-02`、`D-01`、`D-02`、`ST-01`、`R-08`
- Assigned WP：`WP-301`
- Owns：把DB已构造的typed collection放入 `Conversation`
- Does not own：association、query、coverage、event publication、GPUI或任何重新计算
...
- `cargo test -p jaco-conversation`：2 passed；workspace build/test/strict clippy、`cargo fmt` 与 `git diff --check` 通过。
