...
use jaco_core::{ToolPolicySnapshot, ToolSource};
...
pub fn access_requests_for_builtin_tool(
    ...
    let access_requests = match tool {
        ...
        | BuiltinToolName::EditFile => {
            crate::tools::builtin::filesystem::access_requests(tool, arguments, &context)?
        }
        BuiltinToolName::FindPath | BuiltinToolName::Grep => {
            crate::tools::builtin::search::access_requests(tool, arguments, &context)?
        }
    };
