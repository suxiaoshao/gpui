...
参考 TanStack Form 和 React Hook Form，字段和表单都保存 base meta，并提供 derived meta。
...
- 若失败：`is_submitting = false`，`is_submit_successful = false`，聚合报告并聚焦首个 error 字段。
- 若成功：返回 normalized `Output`，`is_submitting = false`，`is_submitted = true`，
