...
Generated accessor、`project` 与 identified-item accessor 都是纯 lens：它们只能读取或修改
cloned `Form::Model` candidate，不能提交 runtime、执行 validation、发出 event 或通知 observer。
无论 root 还是 nested handle，都由最外层 `FormField` 持有唯一 write transaction：执行 lens、
commit 一次、使该 handle 的真实 validation path 失效、运行一次带 scope 的 change-validation，
然后针对 handle 的实际 path 发出一个 `FieldChanged` 并 notify 一次。Lens error 和相等 candidate
都是完全 no-op。
...
### 精确 schema ownership 与 adapter normalization

Adapter field issue 会从完整 stable path 开始规范化。Core 固定先针对本次 model snapshot 解析
path，再检查 validation scope，最后检查精确 owner 的 trigger；它不会使用 root-prefix 或最近
ancestor fallback。因此，即使非法 adapter path 位于本次 field scope 之外，也会变成阻止提交的
internal issue；scope 过滤不能隐藏 adapter/schema contract error。Adapter 或 path mapper 已经
产生的 internal issue 同样不会受 scope/trigger 过滤。非 internal 的 form-level issue 只参与
form scope。

Schema ownership 会递归应用于 group、group 内 array、array item 内 array 以及更深组合：

| Stable path 形态 | Schema owner |
| --- | --- |
| `auth` | 声明的 `auth` group field |
| `auth.username` | nested `username` field |
| `rows` | 声明的 `rows` array field |
| `rows[#id]` | 直接拥有该 item 的 `rows` array field |
| `rows[#id].name` | item model 的 `name` field |

Item segment 只是动态寻址，不会生成 synthetic field schema。Nested array 的 direct item root
只归最近的 array 声明所有，后代继续递归 item model。Item ID 缺失或重复时 path 无法解析。
Projection path 属于 control/application path，不能作为同步 model adapter 的 issue path。
...
Garde 的显示 path 使用 vector position。Generated `GardePathMapper` 会把当前 index 映射到
被验证模型中的稳定 `FormItemId`。Array path 的三种形态都受支持：container（`rows`）、
direct item root（`rows[2]`）与 item leaf（`rows[2].name`），并可递归处理 nested array。
Mapping 只负责转换 display path；随后 core 独立按 schema resolution、scope、trigger 的顺序
进行过滤。内建 adapter 会把完整 mapped report 交给该 normalization，不提前过滤。未知字段、
格式错误或越界 index、无法转换的 ID 和重复 ID 都会变成阻止提交的 internal form issue。
Adapter 不使用 Garde doc-hidden path iterator，也绝不会在最终 `FieldPath` 中保留可变 vector index。
...
Custom adapter 可以把收到的 scope 当作性能提示，但正确性不能依赖自行过滤。Core 会对每个
返回的 field issue 再次按 `schema resolver -> scope -> exact trigger` 规范化。因此非法 stable
path 会作为阻止提交的 internal issue 暴露，而不会因为位于当前 scope 之外就静默消失。
...
`ValueUnavailable` 表示 projected value 不存在，或 identified item 不再恰好匹配一个 ID。
`ItemIdentityChanged` 是另一类错误：identified-item 整项写入或 ID-leaf 写入试图把 captured
stable ID 改成不同或无法转换的 ID。该写入在 cloned candidate 上被拒绝，因此 model、revision、
validation、async task、event 与 notify 都保持不变。
...
Stable ID 在当前 array 内必须唯一，并且不能通过 identified-item handle 改变。寻址 item 缺失
或重复时返回 `FormFieldError::ValueUnavailable`；structural validation 会把 duplicate 或
无法转换的 ID 报告为 blocking issue。通过 identified-item 整项写入或 ID-leaf 写入改成不同或
无法转换的 ID 时返回 `FormFieldError::ItemIdentityChanged`。两种错误路径都是完全 no-op，
库也绝不会选择第一个 duplicate。Whole-array write 仍是显式 add、remove、reorder 或 replace
item 的方式。

Stable identity 是名义语义：在一个 form session 中，`(array path, stable ID)` 表示同一个
logical item。Runtime 不保存 retired-ID history，也不会猜测后续出现的相同 ID 是否代表新对象。
Whole-array write 保留 ID 表示更新该名义 item；改变 ID 表示 remove + insert。应用为新的 logical
item 分配新的 ID。重排会保留寻址，但会使旧 descendant validation result 失效；新的 validation
会把 issue 映射到当前 stable ID。

Validation schema 同样遵循完整 stable path。Array 声明拥有自己的 container，并且只拥有直接
item root；item 下的 leaf 使用 item model 的 leaf schema。Group 和 nested array 递归应用同一
规则，因此每一层自己的 `validate(...)` trigger 彼此独立。
