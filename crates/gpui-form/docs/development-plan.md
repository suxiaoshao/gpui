...
| `number-input-design.md` | number 字段默认使用 `NumberInput` 渲染，以及 raw input 作为 dirty/default 基准的修复方案。 |
...
1. 收口 meta/submit 状态模型：`FieldMeta` / `FormMeta` 不保存合法性或 can-submit 这类第二事实源，
   submit 只依据 final `FormValidationReport` 决定 `Ok`/`Err`，具体设计见 `meta-and-submit-state.md`。
2. 修复 dynamic array 结构性 dirty/default-value 建模，具体设计见 `array-design.md`。
3. 修复 number 字段 dirty/default 建模：`NumberInputBinding` 使用 `NumberInput`，dirty 以 raw input 为基准，
   具体设计见 `number-input-design.md`。
4. 修复 nested group validation routing 的 sibling 同名字段误归属，具体设计见 `validation-routing.md`。
5. 给 array structural edits、number raw dirty 和 validation routing 增加 focused tests：append/remove/move/swap/replace、
   回到默认值、reset rebase、parent/child 同名字段。
6. 继续保持 app-specific validation 在接入 app 内，不把 Provider/MCP/Prompt/Shortcut 的业务规则下沉到
   `gpui-form`。
