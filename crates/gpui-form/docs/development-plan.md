...
`gpui-component` binding 已拆到独立 adapter crate。当前开发计划不再把所有细节堆在一个文件里，完整历史记录
移到 `phase-1-development-plan.md`，专题设计拆到独立文档。
...
| `number-input-design.md` | number raw input dirty/default 的当前修复记录；最终模型见 `binding-architecture.md`。 |
...
2. 继续完善 dynamic array 结构性 dirty/default-value 和 internal array error 语义，具体设计见
   `array-design.md`。
3. 继续给 binding draft、array structural edits、number raw dirty 和 validation routing 增加 focused tests：invalid draft、
   typed-equal draft dirty、normalize writeback、append/remove/move/swap/replace、
