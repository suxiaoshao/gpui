...
3. 修复 nested group validation routing 的 sibling 同名字段误归属，具体设计见 `validation-routing.md`。
4. 给 array structural edits 和 validation routing 增加 focused tests：append/remove/move/swap/replace、
   回到默认值、reset rebase、parent/child 同名字段。
5. 继续保持 app-specific validation 在接入 app 内，不把 Provider/MCP/Prompt/Shortcut 的业务规则下沉到
   `gpui-form`。
