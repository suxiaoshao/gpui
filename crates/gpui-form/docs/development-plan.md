...
2. 给 array structural edits 增加 focused tests：append/remove/move/swap/replace、回到默认值、reset rebase。
3. 继续保持 app-specific validation 在接入 app 内，不把 Provider/MCP/Prompt/Shortcut 的业务规则下沉到
   `gpui-form`。
