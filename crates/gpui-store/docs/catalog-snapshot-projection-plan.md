...
本文只定义 `gpui-store` 在 catalog 场景中的边界；最终 form/component 分离见
`crates/gpui-form/dev/typed-form-store.md`。
...
submit 读取一次 `form.prepare_submit()` 产生的 typed output 和一次 catalog store snapshot，由 app pure resolver 校验
effective value；不读取 UI cache，也不静默替换 unavailable selection。
...
| current typed value、baseline、report、submit runtime | generated `FormStore` | bound control 或 explicit form command |
...
修改 Jaco project/run-settings controllers：catalog selection 只更新 component config/presentation；
form current value 与 baseline 保持不变。submit pure resolver 读取一次 prepared form output 和 catalog snapshot。
