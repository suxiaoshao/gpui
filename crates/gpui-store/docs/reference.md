...
be used by submit or validation as a replacement for the form-owned current
value or the committed store. `StoreBinding<T>` is different: it is
appropriate only when a field intentionally writes back to the same backing
store.
...
```text
committed domain value -> form.rebase(committed value)
catalog snapshot -> component options/projection only
form.prepare_submit() -> typed output -> repository/store command
command success -> committed snapshot reconcile
```

`gpui-store` does not depend on `gpui-form`; a generated form store owns current
editable values, baseline, validation, and submit runtime, while bound components
own only interaction-local state. Catalog/options updates never replace a form
baseline or selected value. Selection and backend observers must
...
The consumer converts `models` into component items. It does not write the
form-owned selection. A submit path separately prepares one typed current
form value and reads one catalog snapshot, then applies product-specific
availability policy in a pure resolver. An unavailable selected value is reported
explicitly rather than silently replaced by the catalog.
