...
pub(super) fn adapt_material_scheme(
    seed_color: &str,
...
#[derive(Clone, Copy)]
pub(super) struct MaterialColorPair {
    pub(super) color: Argb,
    pub(super) on_color: Argb,
}
...
pub(super) fn material_semantic_roles_for_palette(
    scheme: &MaterializedScheme,
...
pub(super) fn hex(color: Argb) -> SharedString {
    color.to_hex().into()
}

pub(super) fn hex_alpha(color: Argb, alpha: u8) -> SharedString {
    format!(
        "#{:02X}{:02X}{:02X}{:02X}",
        color.red(),
        color.green(),
...
pub(super) fn state_layer(container: Argb, content: Argb, alpha: u8) -> SharedString {
